...
describe('installSkills', () => {
  ...
  it('creates command files for claude-code', () => {
    ...
    assert.strictEqual(result.files.length, 31); // 11 core + 11 codex-bridge assets + 8 project-status assets + 1 auto-update hook (no namespace root for commands)
  });
  ...
  it('installs memory module skills when module is enabled', () => {
    ...
    assert.strictEqual(result.files.length, 32); // 11 core + 1 module + 11 codex-bridge assets + 8 project-status assets + 1 auto-update hook (no namespace root for commands)
  });
  ...
  it('creates files for multiple IDEs', () => {
    ...
    assert.strictEqual(result.files.length, 61); // (11 core + 11 codex-bridge assets + 8 project-status assets) * 2 IDEs + 1 auto-update hook (no namespace root for command or toml formats)
  });
