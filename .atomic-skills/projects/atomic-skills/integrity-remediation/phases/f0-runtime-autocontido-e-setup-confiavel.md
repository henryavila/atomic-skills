...
- **Narrative:** A fase F0 permanece ativa com T-001 e T-002 fechadas e `evidence.passed: true`. A T-002 instalou helpers standalone e grupos `*-assets` em profundidade arbitrária, normalizou referências source, rejeitou colisões e publicou o HTML de ajuda; implementação em `dbffedb`, checkpoint de estado em `4058cb9`. Nenhuma tarefa está em voo.
- **Decision log:** Diretórios `*-assets` organizam apenas o source e são removidos da projeção instalada; subdiretórios internos permanecem intactos e duas origens no mesmo destino falham mesmo com bytes idênticos. `tests/install.test.js` entrou em `outputs[]` após autorização explícita do usuário para atualizar as cinco contagens afetadas pelos seis helpers obrigatórios. A API auxiliar inicialmente considerada em `src/config.js` foi removida na revisão G7 por ter um único consumidor.
- **Single nextAction:** Inicie a T-003 escrevendo os testes vermelhos em `tests/project.test.js` e `tests/install-uninstall-roundtrip.test.js`.
- **Verbatim state:** verifier T-002 → `ℹ tests 8`, `ℹ pass 8`, `ℹ fail 0`, seguido de `npm pack --dry-run --json` exit 0; closure CLI → `12 IDE/scope combinations`, `934 rendered files`; `npm test` pós-refactor → `ℹ tests 1656`, `ℹ pass 1648`, `ℹ fail 0`, `ℹ skipped 8`; `npm run validate-skills` → 15 válidas; `npm run check-docs` → exit 0.
- **Uncommitted changes:** nenhuma após os commits `dbffedb` e `4058cb9`; este bloco é o snapshot de retomada.
