...
- **Narrative:** A fase F0 permanece ativa com T-001, T-002 e T-003 fechadas e `evidence.passed: true`. A T-003 substituiu a presença de `.atomic-skills/` por um sentinel estrutural compartilhado pelo router e pelos criadores, tornou o setup idempotente e preservou ledger, hook e estado legacy. Implementação em `ac6c3af`, checkpoint de estado em `8d79ebe`; nenhuma tarefa está em voo.
- **Decision log:** O estado é configurado por índice top-level estruturalmente válido ou por ao menos um plano aninhado validado; legacy é detectado independentemente, inclusive em coexistência, e segue para diagnóstico/migração sem sobrescrita. O ledger continua em `.atomic-skills/manifest.json`: mover `src/manifest.js` foi refutado pelo teste de integração como desnecessário e menos compatível. Setup cria o índice ausente, preserva um índice existente byte por byte e nunca altera ledger ou hook do installer.
- **Single nextAction:** Inicie a T-004 escrevendo o teste vermelho em `tests/consumer-install-e2e.test.js`.
- **Verbatim state:** verifier T-003 pós-commit → `ℹ tests 75`, `ℹ pass 75`, `ℹ fail 0`, `ℹ skipped 0`, `duration_ms 4878.142458`; `npm test` → `ℹ tests 1663`, `ℹ pass 1655`, `ℹ fail 0`, `ℹ skipped 8`, `duration_ms 16350.928458`; router → `22936` bytes; `npm run validate-skills` → 15 válidas; `npm run check-docs` → exit 0; `validate-state.js .atomic-skills` → 165 arquivos, 26 planos e 1 routing config válidos.
- **Uncommitted changes:** nenhuma; este bloco é o snapshot de retomada após `ac6c3af` e `8d79ebe`.
