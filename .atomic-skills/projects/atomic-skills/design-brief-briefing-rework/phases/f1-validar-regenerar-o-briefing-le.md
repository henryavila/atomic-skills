...
nextAction: "Path C escolhido. T-006 done (skill corrigida) + skill re-instalada
  (user-scope agora 12k, com o fix). Proximo = OPERADOR regenera o briefing em
  SESSAO NOVA E CEGA; depois re-rodar o critico e resolver D10 (T-005)."
...
- **Uncommitted changes:** tree limpo (T-006 + skill fix commitados; o `node bin/cli.js install` mutou só `~/.claude`/Cursor/Codex global, fora do worktree). Próxima escrita = o briefing v2 (operador, sessão cega).
