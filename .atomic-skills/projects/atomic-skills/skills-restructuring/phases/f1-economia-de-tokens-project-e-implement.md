...
- **Single nextAction:** F1 commitada (topo de `plan/skills-restructuring`). Aguardando merge de 3 outras worktrees nesta árvore. **Checklist de consolidação (pós-merge), nesta ordem:** (1) mergear as 3 worktrees serial-aware (uma por vez, re-verify no primário — R-XAGENT-03; sem force-resolve em conflito); (2) `npm test` no resultado consolidado; (3) **conectar os hooks de project-status** via setup do skill, com paridade de manifest — instala `.atomic-skills/status/hooks/{session-start,stop,pre-write}.sh`+`config.json` e registra `SessionStart`+`Stop` em `.claude/settings.json` (HOJE ausentes; é por isso que `focus.json` não auto-refresca — o Stop hook é o que regeneraria por turno); (4) `node scripts/refresh-state.js` no estado final; (5) tratar as 8 falhas pré-existentes de contagem (`countSkills`/`installSkills`), que a essa altura devem refletir o catalog PÓS-merge; (6) `phase-done` de F1 — **usuário opta** (intrusive-actions).
