...
nextAction: "F2 EM ANDAMENTO no pacote ~/tooling-installer: MVP Provider/Driver (f83a1f7) + update/re-install semantics 3-hash (0ee4f6d) landed, suíte 50/50, validado por mim. Decisão estrutural: F2 tracejado no pacote, este plano retém só F3. PENDENTE p/ usuário: assimetria data-safety no update (kept-still-desired removido no uninstall vs kept-orphan sobrevive — flagged em docs/design/provider-driver.md). Próximo: F2 config two-tier + runtime-layer, OU iniciar F3 (consome via file:). NÃO rodar phase-done F1 (done/superseded)."
...
- **Single nextAction:** **F2/F3 RE-ESCOPADOS package-first (2026-06-19) — decisões do usuário aplicadas.** [DECIDIDO #1–#4] registrados em `../../PROPOSAL-f2-f3-package-first.md`; o frontmatter F2/F3 do `plan.md` foi reescrito; D1/D7 reframed em `../../design.md` §PIVOT. Fronteira travada: **pacote** = contrato de Provider + Driver + config two-tier (genérico); **consumidor (atomic-skills)** = SkillsProvider + render + flag de idioma + runtime layers; **lib-only**; dependência por **`file:`**. **Pacote validado (por mim, determinístico):** `npm test` → **43/43** (38 base + 5 do Driver), `src/index.js` 17 exports coerentes, README ok. **Decisão estrutural (2026-06-19):** F2 é tracejado/executado **no repo do pacote** `~/tooling-installer` (auto-doc em `docs/design/provider-driver.md`); este plano retém só **F3**. **F2 progresso (pacote):** (1) MVP Provider/Driver (`f83a1f7`) + (2) update/re-install semantics 3-hash (`0ee4f6d`, port da política `--yes` legada: no-clobber + remoção de órfão só não-modificado). Suíte do pacote **50/50**. **⚠️ Pendente p/ usuário:** assimetria data-safety no update (arquivo do usuário ainda-no-desired é removido no uninstall = parity legada; órfão mantido sobrevive) — flagged em `docs/design/provider-driver.md`. **Próximo:** F2 config two-tier → exemplo de runtime-layer, **OU** iniciar F3 no atomic-skills (consome via `file:`). NÃO rodar `phase-done` F1 in-repo (done/superseded pelo pivot).
...
- **Uncommitted changes:** árvore limpa. atomic-skills HEAD `4cc1de8` (pivot record) + plan.md alinhado (commit seguinte). Pacote `~/tooling-installer` HEAD `22bfa99` (repo separado, 38/38). Nada pendente de commit nos dois repos.
