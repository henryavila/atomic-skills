...
- **Single nextAction:** **F2/F3 RE-ESCOPADOS package-first (2026-06-19) — decisões do usuário aplicadas.** [DECIDIDO #1–#4] registrados em `../../PROPOSAL-f2-f3-package-first.md`; o frontmatter F2/F3 do `plan.md` foi reescrito; D1/D7 reframed em `../../design.md` §PIVOT. Fronteira travada: **pacote** = contrato de Provider + Driver + config two-tier (genérico); **consumidor (atomic-skills)** = SkillsProvider + render + flag de idioma + runtime layers; **lib-only**; dependência por **`file:`**. **Pacote validado (por mim, determinístico):** `npm test` → **43/43** (38 base + 5 do Driver), `src/index.js` 17 exports coerentes, README ok. **Decisão estrutural (2026-06-19):** F2 é tracejado/executado **no repo do pacote** `~/tooling-installer` (auto-doc em `docs/design/provider-driver.md`); este plano retém só **F3**. **✅ F2 COMPLETO (pacote, suíte 58/58):** (1) MVP Provider/Driver (`f83a1f7`) + (2) update 3-hash (`0ee4f6d`) + (3) data-safety lado seguro (`7703eac`, arquivo editado sobrevive ao uninstall) + (4) config two-tier `defineInstaller` (`e99cc09`; D2 reinterpretado — config rica é do consumidor) + (5) runtime-layer example symlink (`439fe9a`). **Próximo = F3** (big-bang no atomic-skills): consome via `file:`, monta SkillsProvider+render+idioma+runtime layers sobre `defineInstaller`/Driver, remove `src/kernel/`, paridade round-trip atravessando a dependência (one-way D3). Recomendo decompor F3 (T-F3-1..5) antes de executar. NÃO rodar `phase-done` F1 in-repo (done/superseded pelo pivot).
