...
nextAction: "F2 quase completo no pacote ~/tooling-installer (suíte 55/55, validado por mim): MVP Provider/Driver (f83a1f7) + update 3-hash (0ee4f6d) + data-safety RESOLVIDO lado seguro (7703eac, usuário) + config two-tier defineInstaller (e99cc09). Decisão estrutural: F2 tracejado no pacote, este plano retém só F3. FALTA no F2: só o exemplo worked de runtime-layer (T-F2-4). Depois iniciar F3 (atomic-skills consome via file:, move SkillsProvider+render+runtime layers sobre o Driver, remove src/kernel/). NÃO rodar phase-done F1 (done/superseded)."
