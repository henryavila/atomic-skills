...
nextAction: "F2 EM ANDAMENTO no repo do pacote ~/tooling-installer (MVP Provider/Driver landed, commit f83a1f7, suíte 43/43; validado por mim). Decisão: F2 tracejado no pacote, este plano retém só F3. Próximo: continuar F2 (update semantics / config two-tier / runtime-layer) OU iniciar F3 (atomic-skills consome via file:, move SkillsProvider+render+runtime layers sobre o Driver, remove src/kernel/). NÃO rodar phase-done F1 (done/superseded)."
