...
nextAction: "Operator: project materialize F1 then continue implement (stamp
  automate already on)"
...
tasks:
  ...
  - id: T-002
    ...
    evidence:
      ...
      exitCode: 0
      outputSummary: >-
        skills/shared/implement-automate-maestro.md:7:Layer-2 assert CLI:
        `scripts/assert-automate-gate.js` (must run before C/E/G/I advances).

        skills/shared/implement-automate-maestro.md:10:### HARD-GATE —
        assert-automate-gate before C / E / G / I

        skills/shared/implement-automate-maestro.md:15:node "$(cat
        "$HOME/.atomic-skills/package-root" 2>/dev/null || echo
        .)/scripts/assert-automate-gate.js" \

        skills/shared/implement-automate-maestro.md:36:| **C** | Spawn phase
        writer | **HARD-GATE first:** `assert
parked: []
emerged: []
