...
## Session handoff
> ⛔ **NÃO retome o implement direto. O plano está PAUSADO por um PIVÔ DE DESIGN do operador (abaixo). Resolva o pivô ANTES de qualquer phase-done/F2.**
...
- **Single nextAction:** **Decidir com o operador o timing**, depois agir. A última pergunta minha ficou em aberto quando ele pediu este handoff: **(a)** reabrir o `design.md` AGORA via `atomic-skills:brainstorm` (revisar a premissa central + Decisões 1/3/4/6 para `worktree=feature→PR→develop→main`, levando as 3 implicações acima como insumo) e depois re-planejar F1/F2/F3 via `review-plan`; ou **(b)** registrar a decisão e revisar depois. Recomendação minha: **(a)** — porque **F2 é "integração topology-aware" e o teardown de F1 codificam a suposição "sem PR / base=main"; implementar F2 sobre a premissa velha é trabalho jogado fora.** NÃO carimbar phase-done de F1 nem tocar F2 antes de resolver o pivô.
