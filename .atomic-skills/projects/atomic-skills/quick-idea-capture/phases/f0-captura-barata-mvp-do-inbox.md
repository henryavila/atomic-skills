...
- **Single nextAction:** Quando os 3 Codex retornarem: ler `git -C /home/henry/.wt-qic-t-00N diff` de cada um, merge-back SERIAL (T-001 → T-002 → T-003) com re-run do verifier na primária mesclada antes de cada `done`.
- **Dispatch em voo (2026-06-09):** worktrees `impl/qic-t-001..003` em `/home/henry/.wt-qic-t-00{1,2,3}` (base `53dbcc3`); briefings `/tmp/qic-dispatch/briefing-t-00N.md`; outputs `/tmp/qic-dispatch/out-t-00N.md`; exit codes `/tmp/qic-dispatch/exit-t-00N`. Comando: `timeout 600 codex -a never exec -c model_reasoning_effort=high --sandbox workspace-write --skip-git-repo-check --ephemeral -o <out> - < <briefing>` com cwd na worktree.
