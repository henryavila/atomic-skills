...
| `reconcile` fechando task/criterion | Detector encontrou sinal e o fechamento foi autorizado por verifier ou ack humano (`done` e a closure authority de task; `reconcile` e so o path disparado por detection drift). | Candidato tem verifier e ele nao passou; snapshot de escrita ficou stale; candidato mudou desde a deteccao. | Candidato sem verifier pode usar `Mark done` manual; `Still open` reseta so o anchor suportado (task.`lastUpdated`; criterion → initiative.`lastUpdated` — nunca `lastUpdated` em ExitCriterion). | Corrigir o verifier ou escolher `Still open`; depois rerodar `reconcile`. |
