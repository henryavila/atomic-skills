/* global React, window, StatusGlyph, HighlightBadge, TagChip */

// ── PhaseCard ─────────────────────────────────────────────────────────────
// The atom of the plan view. Three density modes, three active-emphasis
// modes, used identically in solo and parallel-group contexts.

const PhaseCard = ({
  phase, onOpen,
  density = 'baseline',          // 'tight' | 'baseline' | 'expanded'
  activeEmphasis = 'edge-scan',  // 'edge' | 'edge-scan' | 'glow'
  inParallel = false,
  hashTarget = false,            // currently selected via hash nav
}) => {
  ...
  // Compose design-system texture classes
  ...
  return (
    <button
      ...
      style={{
        ...
        transition: 'background 120ms var(--ease-out), border-color 120ms var(--ease-out), transform 120ms var(--ease-out), box-shadow 120ms var(--ease-out), opacity 120ms var(--ease-out)',
        scrollMarginTop: 80,
      }}
      onMouseEnter={(e) => {
        e.currentTarget.style.background = 'var(--bg-elevated)';
        if (!hashTarget) e.currentTarget.style.borderColor = 'var(--border-bright)';
        e.currentTarget.style.transform = 'translateY(-1px)';
        if (isDone) e.currentTarget.style.opacity = '0.95';
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.background = 'var(--bg-surface)';
        if (!hashTarget) e.currentTarget.style.borderColor = 'var(--border-default)';
    ...
    >
      ...
      {/* Row 1: glyph · id · title · highlights */}
      <div style={{
        ...
        <span style={{
          ...
          color: isActive ? 'var(--status-active)' :
                 isDone   ? 'var(--status-done)'   :
                            'var(--fg-muted)',
          letterSpacing: '0.02em', whiteSpace: 'nowrap', flex: 'none',
          minWidth: 26,
        }}>{phase.id}</span>
        <span style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          color: isDone
            ? 'var(--status-done)'
            : (phase.gates.met < phase.gates.total ? 'var(--status-blocked)' : 'var(--status-done)'),
          whiteSpace: 'nowrap',
        }}>
      ...
      {showProgress && (
        <div style={{
          display: 'flex', height: 3, marginTop: 10,
          marginLeft: 34, marginRight: 6, gap: 1,
      ...
      {/* Row 5: next action or completion meta */}
      {showNext && (
        isDone ? (
          <div style={{
          ...
          <div style={{
            fontFamily: 'var(--font-sans)', fontSize: 12.5, color: 'var(--fg-muted)',
            marginTop: 8, marginLeft: 34,
          }}>
...
// ── Parallel group (three variants) ───────────────────────────────────────

const ParallelGroup = ({ phases, onOpen, density, activeEmphasis, variant = 'bracket', hashId }) => {
  // 'bracket'    — v3 baseline: left rail + eyebrow
  // 'container'  — explicit pill enclosing both, with chain glyph between
  // 'chain'      — adjacent cards joined by chain icon, faint shared tint
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {phases.map((p, idx) => (
            <React.Fragment key={p.id}>
              <PhaseCard phase={p} onOpen={onOpen} inParallel
              ...
              {idx < phases.length - 1 && (
                <div style={{
                  ...
                  <span style={{ flex: 1, height: 1, background: 'color-mix(in srgb, var(--status-emerged) 25%, transparent)' }} />
  ...
  if (variant === 'chain') {
    return (
      <div style={{
        position: 'relative',
        background: 'color-mix(in srgb, var(--status-emerged) 4%, transparent)',
        borderRadius: 8,
        padding: 6,
      }}>
        <div style={{
          ...
          <span style={{
            fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 600,
            color: 'var(--status-emerged)', letterSpacing: '0.08em', whiteSpace: 'nowrap',
          }}>∥ PARALLEL · {phases.length} PHASES</span>
          <div style={{ flex: 1, minWidth: 0, height: 1,
  ...
  return (
    <div style={{ position: 'relative', paddingLeft: 16 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {phases.map(p => (
          <PhaseCard key={p.id} phase={p} onOpen={onOpen} inParallel
                     density={density} activeEmphasis={activeEmphasis}
                     hashTarget={hashId === p.id} />
        ))}
      </div>
...
window.PhaseCard = PhaseCard;
window.ParallelGroup = ParallelGroup;
