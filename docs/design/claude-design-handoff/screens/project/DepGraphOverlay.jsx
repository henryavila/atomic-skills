...
// Hand-drawn SVG of the phase DAG. Only mounts when `open` is true — this
// is the "lazy load" the brief asks for; the cost is paid on first click.

const DepGraphOverlay = ({ open, plan, onClose, onOpenPhase }) => {
  if (!open) return null;

  // Topological column assignment via Kahn's algorithm.
  const positions = React.useMemo(() => {
    ...
    plan.deps.forEach(d => {
      if (inDeg[d.to] != null) { inDeg[d.to]++; adj[d.from].push(d.to); }
    });
    const col = {};
    ...
    // Row by track index.
    const trackIdx = {};
    plan.tracks.forEach((t, i) => { trackIdx[t.id] = i; });
    const maxRow = Math.max(0, plan.tracks.length - 1);

    // Pixel layout.
    const COL_W = 150, ROW_H = 72, PAD_X = 96, PAD_Y = 56;
    const positions = {};
    phases.forEach(p => {
      const c = col[p.id] != null ? col[p.id] : 0;
      const r = trackIdx[p.track] != null ? trackIdx[p.track] : 0;
      positions[p.id] = {
        x: PAD_X + c * COL_W,
        y: PAD_Y + r * ROW_H,
        col: c,
    ...
    return { positions, width, height, PAD_X, PAD_Y, COL_W, ROW_H, maxCol, maxRow };
  }, [plan]);

  const { positions: pos, width: W, height: H, PAD_X, PAD_Y, COL_W, ROW_H, maxCol, maxRow } = positions;

  const phaseStatusColor = (s) => {
    if (s === 'done') return 'var(--status-done)';
    if (s === 'active') return 'var(--status-active)';
    return 'var(--fg-muted)';
  };

  // Edge: orthogonal connector with a small mid-curve.
  const renderEdge = (d, i) => {
    const a = pos[d.from], b = pos[d.to];
    if (!a || !b) return null;
    const x1 = a.x + 42, y1 = a.y;
    const x2 = b.x - 42, y2 = b.y;
    // Curved S using cubic bezier
    const dx = (x2 - x1) * 0.5;
    ...
    <div style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <span className="t-eyebrow" style={{ color: 'var(--fg-default)' }}>⌬ Dependency graph</span>
          <span style={{
            fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg-subtle)',
            padding: '2px 8px', borderRadius: 999,
            background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)',
          }}>{plan.phases.length} nodes · {plan.deps.length} edges</span>
          <div style={{ flex: 1 }} />
          <span style={{
            fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-subtle)',
          }}>columns: topological depth · rows: track</span>
          <button onClick={onClose} aria-label="Close" style={{
            all: 'unset', cursor: 'pointer', color: 'var(--fg-muted)',
            fontFamily: 'var(--font-mono)', fontSize: 18, padding: '0 4px', marginLeft: 8,
          }}>×</button>
        </div>
        ...
        <div style={{
          ...
          <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}
            ...
            {/* Row guide labels (tracks) */}
            {plan.tracks.map((t, i) => (
              <g key={t.id}>
                <line x1={PAD_X - 28} x2={W - PAD_X / 2}
                      y1={PAD_Y + i * ROW_H} y2={PAD_Y + i * ROW_H}
                      stroke="var(--border-subtle)" strokeDasharray="2 4" opacity="0.5" />
                <text x={28} y={PAD_Y + i * ROW_H + 4}
            ...
            {/* Col guide labels (topological depth) */}
            ...
            {plan.deps.map(renderEdge)}

            {/* Parallel-pair backdrop pill */}
            {(() => {
              const pairs = new Set();
              const rendered = [];
...
window.DepGraphOverlay = DepGraphOverlay;
