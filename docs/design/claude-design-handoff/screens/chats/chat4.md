...
_Started 2026-05-20 11:30 UTC_
...
Now let me write the extended skills data:

_[tool: write_file]_

Now the main HelpView with all the rich behavior:
...
Help screen is ready at **Help.html**.

**What's there:**
- Dense card grid (≥280px columns) → collapses to a 1-col list + sticky detail panel when a card is selected, so it feels like a card detail opening in place, not a takeover.
- Search input filters across name, purpose, and when-to-use text live (sub-millisecond at 12 skills, will stay well under 50ms past 30+). `/` focuses it. Result count appears inline.
- Filter pills: **All · In repo · Available** with live counts.
- Cards show active-dot (`green = in repo`, `dim = available`), name, summary, first when-situation + "+N more", and an inline copy-command row.
- Detail panel shows invocation, **WHEN TO USE** (+ markers, done-green), **WHEN NOT TO USE** (− markers, warn-amber), each example as its own copyable row, and **RELATED SKILLS** as chips that swap the detail in place.
- Copy buttons flip to a green ✓ for ~1.4s on success. Works in non-secure-context preview iframes via a fallback.
- Keyboard: Tab through cards, Enter/Space opens, ↑↓ moves selection while detail is open, Esc closes. Focus ring is the system's cyan one.
- Edge cases handled: a 12th skill (`echo`) is marked **metadata incomplete** and still copies; toggle "Simulate zero skills" in Tweaks for the empty state with a setup-guide CTA; "no results" state when filters exclude everything.

**Tweaks panel** (toggle from the toolbar): Density (comfortable/dense), canvas texture, demo banner on/off, and the simulate-empty switch.
...
Fixed both eyebrow wrapping issues — added `whiteSpace: 'nowrap'` + `flex: 'none'` to the "IN REPO / AVAILABLE" badge in the detail header and to the `Section` eyebrow labels.
