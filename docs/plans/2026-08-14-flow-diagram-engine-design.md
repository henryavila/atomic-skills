...
**Celular / toque (2026-08-14):** `100dvh` + `viewport-fit=cover` + safe-area. ≤860px ou `pointer:coarse` sem hover: abas em 3 colunas (34px, sem `kbd`); dock de zoom/PDF/tema embaixo (32px); hint de teclado some. Um dedo = scroll nativo (`touch-action: pan-x pan-y`); pinça = zoom do SVG (não da página; `maximum-scale=1`). Zoom do telefone guarda em `as-flow-zoom:<slug>:m` (não herda o 100% do desktop). Primeira visita no telefone = caber na largura. Toque na lifeline mostra o ator. PDF no iOS abre o blob (Safari ignora `download`).

**Vários processos (2026-08-14):** um plano tem N arquivos de processo; cada um gera Sequência + Fluxo + Estados. Chrome: botão `Processo` (nome atual + contagem) abre lista com busca, rolagem e persistência (`as-flow-process:<plan>` + `#p=<slug>`). Funciona com 2 ou com dezenas. Zoom/PDF são do processo visível. JSON extra em `docs/plans/preview-processes/` entra na prévia.
...
**Tema (2026-08-14):** Sistema / Claro / Escuro. Default = `prefers-color-scheme` (`data-theme` ausente). Override = `html[data-theme=light|dark]` + `localStorage.as-color-scheme`. Ícone no dock (ao lado do zoom) cicla automático → claro → escuro; sem a faixa de três botões. Look A usa **nomes** de token, não hex — a paleta light em `site/assets/ds.css` (`--bg-canvas: #f4f6fa`, `--fg-default: #12161d`) re-pinta o mesmo SVG. HTML gerado permanece determinístico (o tema é runtime).
