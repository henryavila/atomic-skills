...
> **Manual-validation gate invariant.** Every Plan and every standalone Initiative carries exactly one reserved `G-MANUAL` exit gate — a `kind: manual` criterion that is the LAST gate before close (created by `atomic-skills:project-plan`; see its *Mandatory final manual-validation gate*). For a Plan it sits on the **terminal phase's** `exitGate.criteria[]`; for a standalone Initiative it sits in `exitGates[]`. It is **non-deferrable**: its only terminal status is `met`, set after explicit human sign-off via the `kind: manual` verifier flow. `phase-done` (plan-done branch) and `archive` MUST NOT complete the close while `G-MANUAL` is `pending`, and MUST NOT coerce it to `deferred` — if the user does not confirm, the close is ABORTED and the Plan/Initiative stays open. If a Plan or standalone Initiative reaches close without a `G-MANUAL` gate (e.g. created before this rule), inject it on the spot, run the manual verifier, and only then close.
...
5. If any criterion is still `pending` after iteration: ask "Some gates remain pending. Mark phase done anyway? (y/N)". On `n`, leave initiative in `active` state and stop. On `y`, document the override (set `deferredReason` on the remaining criteria) and proceed to step 6. **The reserved `G-MANUAL` gate is exempt from this blanket override** — it can never be swept to `deferred`. If it is still `pending` here, run its `kind: manual` verifier individually (per its non-deferrable exception); if the user does not confirm, abort instead of proceeding.
...
7. **Advance the plan** — call `src/transition.js`:`proposeAdvance(plan, phaseId)` to decide what's next. The function returns one of three shapes:
   - `{ kind: 'plan-done', eligible: [] }` — no more eligible phases, so this is the plan's terminal phase. **Before** offering to mark the plan `status: done`, enforce the *Manual-validation gate invariant*: the terminal phase carries the reserved `G-MANUAL` gate. Run it now via the `kind: manual` verifier flow (its non-deferrable exception applies). If the user does not confirm (`y`), ABORT — leave the plan `status: active`, do NOT mark this phase `done`, and stop. Only once `G-MANUAL` is `met` may you offer to set the plan `status: done` and `archive` it. If the terminal phase has no `G-MANUAL` gate (legacy plan), inject it first (see project-plan's *Mandatory final manual-validation gate*), then run it.
...
**Exception — the reserved `G-MANUAL` gate.** When the criterion `id` is `G-MANUAL` (the final manual-validation gate), it is **non-deferrable** (see *Manual-validation gate invariant*). Do NOT offer `defer`. Ask only: "Final manual validation — confirm the delivered work meets the goal? (y/n)". On `y`: write `evidence` and set `status: met`, `metAt: <now>`. On `n`: leave `status: pending` and **abort the enclosing close** (`phase-done`/`archive`) — never `deferred`, never `met` without an explicit `y`. The Plan/Initiative stays open until the user validates.
...
2. **Plan archival**:
   - **Manual-validation gate.** Archiving a plan is a close, so enforce the *Manual-validation gate invariant* unless the plan is already `status: done` (it was closed via plan-done, where `G-MANUAL` was already met). Ask: "Is this plan being archived as **completed** or **abandoned**?" On **completed**: run the terminal phase's `G-MANUAL` via the `kind: manual` verifier; if the user does not confirm, ABORT the archive. On **abandoned**: proceed, but do NOT mark `G-MANUAL` as `met` or `deferred` — leave it `pending` and record a one-line abandonment reason in the plan body's notes. `G-MANUAL = met` must always mean a real human validation happened.
...
3. **Initiative archival**:
   - **Resolve open exit gates first** (applies to BOTH standalone and plan-anchored initiatives — standalone has no `phase-done`, so this is the only place its gates get closed). For each `exitGates[]` entry whose `status` is not already `met` or `deferred`: run its `verifier` per the **Verifier execution patterns** (or ask the user when `kind: manual`), then set `status: met` (`metAt: <now>`, plus `evidence` when a verifier ran) on pass, or `status: deferred` (`deferredReason`) when the user skips it. **Never set `done`** — that is a Task status; gate status is `pending`/`met`/`deferred` only (see the *Gate status invariant* above). If the user wants to archive without verifying, mark the remaining gates `deferred` with a reason — do not leave them `pending` and do not coerce them to `done`. **The reserved `G-MANUAL` gate is the one exception** (see *Manual-validation gate invariant*): it is non-deferrable and MUST be `met` before the archive completes. Run its `kind: manual` verifier; if the user does not confirm (`y`), ABORT the archive — do not move the file, do not set `status: archived`. The initiative stays open until the user validates. If a standalone initiative has no `G-MANUAL` gate (legacy file), inject it (per project-plan's *Mandatory final manual-validation gate*) and run it before archiving.
