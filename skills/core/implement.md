...
**Summary:** one code-only phase writer per phase → merge → post-merge `done` → evaluation agent + `evaluationGate` → **distill lessons + operator ratify + `lessonsState` (recorded+path or explicit none)** → `phase-done` with `review-code --mode=both` → **`recordPhaseDonePause` (`awaiting-operator-advance`)** → **operator `clearContinue` (`operator-continue`)** → next phase (materialize if descriptor-only) → plan-end **`external-both`** (codex|grok|claude) + user validation via durable stamp. **Assert gates at C/E/G/I** via `assert-automate-gate` before each advance (phase-done requires evaluation **and** lessons); **advance the maestro cursor** on every A–I boundary so the step matches. **Never skip skill steps** under automate (evaluation, lessons, review, assert).
...
- **Automate (`isAutomateActive`):** Fixed order — all phase tasks `done` → **evaluation agent** (read-only; see `skills/shared/implement-phase-evaluator.md`) → **persist evaluationReport under `.atomic-skills/reviews/`** → stamp authentic `evaluationGate` (`passed` + non-empty **`reportPath`**, or operator **`operatorSkip`**+reason / disposition — never forge) → **distill lessons** (project-transitions G1: draft → **`Proposed lessons:`** → **operator ratify**) → stamp **`lessonsState: recorded` + `lessonsPath`** (or **`lessonsState: none`** for clean phase — silence is not an answer; `phaseLessonsAllowsClose` / `buildLessonsState`) → **`review-code --mode=both`** (default under automate) → stamp honest **`reviewGate`** (`mode: both` + `at` + `reviewFile`; local only with **`overrideReason`**) → **then** **`assert-automate-gate --gate phase-done` (exit 0 required — evaluation + lessons + review)** → **then** terminal `phase-done` writes. Do not assert before review; do not skip evaluation, lessons, review, or the assert. On evaluation blocker/critical, reopen/fix (max 2 re-dispatches) before phase-done. Evaluation pass does not finalize. Orchestrator writes dispositions to the durable decisions log.
