...
4. **Clear path:** `implement --clear-execution-mode` (parse sets `clearExecutionMode: true` → `isAutomateActive` false) **and** remove the stamp with `clearExecutionModeStamp(plan)` (delete `executionMode` from frontmatter), persist, record in the decisions log. **HARD-GATE:** before honoring clear or falling into Mode 1, check the selected plan's writer lease (`readLeaseResult` / `isLeaseBlocking`) — refuse clear while lease is `active` or `malformed` (or merge mid-flight). Explicit `--mode=1` / non-automate CLI also overrides stamp for that session without deleting it; full leave-automate requires the clear/unstamp path **after** the lease is clean.
...
3. **Writer-lease HARD-GATE (always for the selected plan):** refuse resume **and** refuse `--clear-execution-mode` / Mode-1 entry if a phase-writer lease is `active` **or** `malformed` (see `src/writer-lease.js` — `readLeaseResult` / `isLeaseBlocking` / `hasActiveLease`; malformed ≠ missing) or a sibling phase merge is mid-flight. Acquire via exclusive create (`acquireLeaseFile` / `wx`); clear only with matching owner token after sync-wait + claim collect + merge settle.
