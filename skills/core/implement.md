...
## Mode 2 — Codex-only cross-provider execution (deferred, default OFF)

**Mode 1 (above) is the unconditional default** — no cost gate, no provider config. Mode 2 offloads *mechanical, independently-verifiable* execution onto a foreign WRITING model (OpenAI Codex via a `--sandbox workspace-write` extension of the read-only bridge, inside an isolated `git worktree` per `skills/shared/worktree-isolation.md`) to conserve the scarce flat-rate Opus weekly quota — **not** per-token dollars (a non-goal under flat-rate). Full purpose + gate in `docs/design/project-orchestrator/03-execution-mode2-spec.md`.

Mode 2 is **NOT enabled in this version.** It ships only after two preconditions are specified+tested: the worktree **merge-back** procedure (operator-prompted serial: pass-in-worktree → merge onto primary → re-run the verifier on the merged tree → only then done; R-XAGENT-03) and the operator-supplied routing config `.atomic-skills/status/routing.json` (R-XAGENT-10; absent file ⇒ Mode-1 defaults). Until both land, a request to "use Mode 2" runs Mode 1 and records the reason — never a silent collapse, never a failure (R-EXEC-30). When it does ship: Opus PLANS+REVIEWS only and NEVER executes; routing is per-TASK never per-feature; the executed verifier is the escalation judge; the cheap/Codex executor self-**checks** but never self-**certifies** (`verify-claim`).
...
- "Mode 2 would be faster, let me route this batch to Codex." → Mode 2 is not enabled in this version (no merge-back + routing config). The request runs Mode 1 with a recorded reason.
...
| "Mode 2 is built, I'll just point it at Codex" | Mode 2 needs merge-back + routing.json first; until then it degrades to Mode 1 with a recorded reason, never a silent collapse (R-EXEC-30). |
