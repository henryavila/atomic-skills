...
## Mode 2 — Codex-only cross-provider execution (built, default OFF, fenced from the live tree)

**Mode 1 (above) is the unconditional default** — no cost gate, no provider config. Mode 2 offloads *mechanical, independently-verifiable* execution onto a foreign WRITING model (OpenAI Codex via a `--sandbox workspace-write` extension of the read-only bridge, inside an isolated `git worktree` per `skills/shared/worktree-isolation.md`) to conserve the scarce flat-rate Opus weekly quota — **not** per-token dollars (a non-goal under flat-rate). Opus PLANS+REVIEWS only and NEVER executes; routing is per-TASK never per-feature; the executed verifier is the escalation judge; the cheap/Codex executor self-**checks** but never self-**certifies** (`verify-claim`). The full lane mechanics — the firing threshold, the two-question gate, the per-task tier map + runtime dispatch gate, the handoff work-order contract (the executor RECEIVES intent), the escalation cascade, and the sidecar telemetry — live in **`skills/shared/mode2-codex-lane.md`**; purpose + worth-it verdict in `docs/design/project-orchestrator/03-execution-mode2-spec.md`.

Mode 2 is **default OFF and FENCED OUT of the live tree.** It runs ONLY when BOTH gates hold (R-EXEC-44): `.atomic-skills/status/routing.json` has `mode2Enabled: true` + `codexLane.enabled: true` (validated by `meta/schemas/routing.schema.json`; **absent file ⇒ Mode-1 defaults**) AND the operator opts in per-invocation. Codex write-mode is proven on a **throwaway git repo, never the live `.atomic-skills/` tree** (canon Decision #11). Any un-gated "route this to Codex" — or a failed precondition (Codex unauthenticated, no dispatchable lane; on Gemini the investigator tool is read-only so Mode 2 = Codex-only) — runs Mode 1 for that work with the reason **recorded**, never a silent collapse, never a failure (R-EXEC-30).
...
- "Mode 2 would be faster, let me route this batch to Codex." → Mode 2 runs ONLY when `routing.json` enables it (`mode2Enabled` + `codexLane.enabled`) AND the operator opts in per-invocation, and never against the live tree. Absent that, the request runs Mode 1 with a recorded reason — "faster" is not the enable surface.
- "I built and tested this gate, so I know it's safe — the throwaway-repo fence is for people who don't understand the feature, not me." → The fence exists FOR the confident author under a deadline — that operator is exactly who corrupts the live tree, so "I understand it" is not an exemption, it is the risk the fence names. Competence is not the enable surface; flip `routing.json` + opt in + use a throwaway repo, or run Mode 1.
- "I'll flip mode2Enabled back off when I'm done, so pointing Codex at the live tree is only temporary — no state is actually at risk." → The transient window IS the risk: a foreign writer let loose on the live `.atomic-skills/` state can corrupt it inside that window, and restoring the config afterward un-does nothing (the live state is not reliably git-restorable — it carries uncommitted edits — and there's no throwaway boundary to absorb it). "Temporary" and "I'll revert it" do not lift the fence — prove it on a throwaway repo.
...
| "Mode 2 is built, I'll just point it at Codex" | Built ≠ on. It runs only behind `routing.json` (`mode2Enabled` + `codexLane.enabled`) + a per-invocation opt-in, and never against the live `.atomic-skills/` tree (fenced to a throwaway repo). No enable surface ⇒ Mode 1 with a recorded reason (R-EXEC-44/30). |
| "I built+tested it, so the throwaway-repo fence isn't for me" | The fence is FOR the confident author under deadline — that is exactly who corrupts the live tree. Competence is not the enable surface; "I know it's safe" is the rationalization the fence exists to override (Decision #11). |
| "I'll flip the config back after — it's only temporary, no state is at risk" | The transient window IS the risk: a foreign writer can corrupt the live state inside it, and reverting the config un-does nothing (uncommitted state is not reliably git-restorable, no throwaway boundary). Temporary ≠ safe — prove it on a throwaway repo. |
