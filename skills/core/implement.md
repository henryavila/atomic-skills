...
7. **phase-start package + operator validate-only (shared ritual at Step B and Step H).** Fixed order under automate: (1) present package as **draft only** (phase **objective** + **task list** id+title + **drafted** `businessIntent` — **no durable BI write** while descriptor-only); (2) operator **validate-only** on the **BI spine** (field edit allowed; durable **title** renames require **re-spec**/sidecar — titles in the package are advisory; **blank form forbidden**); (3) explicit **ratify** token; (4) **then** materialize from sidecar with the **ratified** spine if descriptor-only (materialize accepts pre-ratified complete spine — does **not** write BI before ratify); (5) only then work-order → lease → **fresh** phase writer. **Blank-fill BI** and silent auto-PASS forbidden. **Do not auto-spawn** without package ratify. **Forbid reusing previous writer context.** After Step H ratify, **re-enter Step B** (work-order) then **C** — do not skip work-order. `phase-done` under automate does **not** blank-form materialize the successor — sets nextAction to the package ritual; Step H/B owns materialize after ratify. At every phase boundary, session handoff **single nextAction** is **mandatory** (prefer `present phase-start package for F{N} validate-only` / `await package ratify` / `spawn fresh writer after ratify` — not bare operator `materialize` when host orchestrates package→ratify→materialize).
...
9. **Role banner at load-bearing stops.** At pure-maestro Steps **A**, **C**, **D.5**, **E**, **F**, **G**, and **H**, emit a one-line **role banner** stating host-thin maestro (dispatch/merge/verify/state only — no product source, no product entrypoint).
...
Also: phase writer `{{READ_TOOL}} skills/shared/implement-phase-writer.md`; isolation/lease `{{READ_TOOL}} skills/shared/worktree-isolation.md` + `src/writer-lease.js`; evaluation `{{READ_TOOL}} skills/shared/implement-phase-evaluator.md`; **decision log** (automate durable append + decision-review operator-only PASS) `{{READ_TOOL}} skills/shared/implement-decision-log.md` + `src/decision-log.js` (`appendDecision` / `listDecisions`); STOP helpers `src/automate-orchestrator-gates.js` (`canSpawnHostThinPhaseWriter` = Step C gate under automate, `canSpawnPhaseWriter`, `canCloseTasksFromClaims`, `canRunPhaseDone` = evaluationGate **AND** decisionReview under automate, `canFinalizeOrArchive`). Machine preflight: under `executionMode: automate`, `canRunPhaseDone` / `assert-automate-gate --gate phase-done` fail closed without both a valid `evaluationGate` and operator-stamped `decisionReview` (`status=passed` + `verifiedAt`).

**Summary (host-thin):** **role banner** at Steps A / C / D.5 / E / F / G / H stating host-thin maestro → **Step B / H shared phase-start package** (draft only: phase **objective** + **task list** id+title + **drafted** `businessIntent`; operator **validate-only** on BI spine; titles advisory until re-spec; blank form forbidden; **no durable BI write before ratify**; after ratify materialize with ratified spine if descriptor-only; then work-order → lease → spawn; **forbid reusing previous writer context**) → one **fresh** phase agent per phase → merge → post-merge `done` (preserve active handoff until phase-done) → evaluation agent + `evaluationGate` → **decision-review operator PASS** (explicit operator token same turn; host/agent never write `decisionReview` PASS without it; FAIL stamps `failed` and does not advance `currentPhase`) → stamp `decisionReview` → `canRunPhaseDone` preflight → `phase-done` with `review-code --mode=both` (under automate: advances pointer + nextAction to package ritual — **does not** blank-form materialize successor) → Step H package → ratify → materialize if needed → re-enter B work-order → C fresh spawn → plan-end **`external-both`** (codex|grok|claude) + user validation via durable stamp.
...
- **Automate (`isAutomateActive`):** Fixed order — all phase tasks `done` → **evaluation agent** (read-only; see `skills/shared/implement-phase-evaluator.md`) → stamp `phases[].evaluationGate` via `buildEvaluationGate` → **decision-review operator PASS** (mandatory manual hardgate; host/agent **never** write `decisionReview status=passed` without explicit operator token in the **same turn**, e.g. `decision-review PASS` / `ratify decision-review`; evaluation agent and review-code receipt **never** substitute) → stamp `phases[].decisionReview` via `buildDecisionReview` → preflight `canRunPhaseDone` (evaluationGate **AND** decisionReview) → then `phase-done` with `review-code --mode=both` (default under automate). **FAIL path:** stamp `decisionReview status=failed` and **do not advance** `currentPhase`. Do not skip the evaluation agent, the evaluationGate stamp, or auto-PASS decision-review. On evaluation blocker/critical, reopen/fix (max 2 re-dispatches) before phase-done. Orchestrator writes re-dispatch / skip / disposition / scope-exit to the durable **decision log** (`skills/shared/implement-decision-log.md`, `src/decision-log.js`) **before continuing**. **After phase-done under automate:** advance the descriptor pointer and set handoff **single nextAction** to the phase-start package ritual (e.g. `present phase-start package for F{N} validate-only`) — do **not** blank-form `project materialize` the successor as a separate invent-BI UX. Step H owns: present package as **draft only** (objective + task list id+title + **drafted** `businessIntent`; **no durable BI write yet** if descriptor-only) → operator **validate-only** on BI spine (titles advisory; durable renames require re-spec; blank form forbidden) → explicit **ratify** → **then** materialize from sidecar with **ratified** spine if descriptor-only → **re-enter Step B** (work-order) → **C** spawn **fresh** phase agent; **forbid reusing previous writer context**. Do **not** spawn a writer and do **not** silent host-code the new phase until package ratify (+ materialize when required).
...
- "I will just run compose myself" / "I'll just run compose myself to unstick the phase."
...
Under `--mode=automate` / pure maestro **host-thin**: host never edits product source and never runs a **product entrypoint** (compose / build_edl / app servers / product diagnostics / `npm run dev` style) except **verbatim** task/exit-gate verifiers; **allow host class** (open) covers plan-branch git-ops, Atomic Skills state scripts (done/phase-done/refresh-state/validate-state/assert-automate-gate/decision-log), review-code, and verifiers copied from `task.verifier` — not a closed allowlist; phase-start package ritual (draft objective + tasks + drafted BI → validate-only → ratify → materialize with ratified spine if needed → work-order → fresh writer); one fresh phase agent per phase; orchestrator owns merge, post-merge re-verify, `done`, evaluation agent, decision-review manual hardgate (operator PASS only — agents never write PASS), and `phase-done` (no blank-form successor materialize under automate); never self-certify; never silent Mode-1 fallback or silent auto-PASS; on failure re-dispatch **fix agent** (not host product debug); finalize/archive require `planEndReviewOk` and that the user validates implementation.
