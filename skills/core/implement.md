...
7. **phase-start package + operator validate-only.** Before spawning the phase agent, present a **phase-start package**: phase **objective**, **task list** (id + title), and a **drafted** `businessIntent` **draft** for **validate-only** before spawn. Operator role is **validate-only** for task titles and `businessIntent` (edit allowed, then ratify). **Blank-fill BI** (dumping an empty form) and silent auto-PASS are forbidden. **Do not auto-spawn** without operator ratify of the phase-start package. After `phase-done`, if the successor needs a package (descriptor-only / unratified BI), host must **not** spawn a writer until materialize + ratify — handoff **nextAction** points at `project materialize <phase-id>`.
...
**Summary (host-thin):** **role banner** at Steps A / C / D.5 / E / F / G / H stating host-thin maestro → **phase-start package** (objective + tasks + **drafted** BI **draft**; operator **validate-only**, no spawn until ratify) → one fresh phase agent per phase → merge → post-merge `done` → evaluation agent + `evaluationGate` → **decision-review operator PASS** (explicit operator token same turn; host/agent never write `decisionReview` PASS without it; FAIL stamps `failed` and does not advance `currentPhase`) → stamp `decisionReview` → `canRunPhaseDone` preflight → `phase-done` with `review-code --mode=both` → next phase: if successor descriptor-only, **nextAction** = materialize + phase-start package (do **not** spawn writer until ratify) → plan-end **`external-both`** (codex|grok|claude) + user validation via durable stamp.
