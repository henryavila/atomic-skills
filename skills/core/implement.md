...
7. **phase-start package + operator validate-only (Step H ritual).** Before spawning the phase agent, present a **phase-start package**: phase **objective**, **task list** (id + title), and a **drafted** `businessIntent` **draft** for **validate-only** before spawn. Operator role is **validate-only** for task titles and `businessIntent` (edit titles and BI allowed, then ratify; **blank form forbidden**). **Blank-fill BI** (dumping an empty form) and silent auto-PASS of drafted `businessIntent` are forbidden. **Do not auto-spawn** without operator ratify of the phase-start package. **After ratify**, spawn a **new** phase agent with **fresh** context only — **forbid reusing previous writer context** (no prior-writer session, no host chat history). At every phase boundary, session handoff **single nextAction** is **mandatory** (exactly one concrete step). If the successor is **descriptor-only**, automate **orchestrates materialize from sidecar** and attaches the **drafted** BI into the validation package — operator validates the drafted package only (does not invent BI from blank); host must **not** spawn a writer and must **not** silent host-code the new phase until package ratify.
...
2. If the parent plan phase descriptor exists but the matching initiative file is absent, this is a **descriptor-only phase**. **Refuse writer spawn / product coding** (HARD-GATE). Under **automate** (`isAutomateActive`): **orchestrate materialize from the phase source sidecar** as part of the **phase-start package** prep, attach a **drafted** `businessIntent` into the validation package, and present it for operator **validate-only** (operator does **not** invent BI from blank; no silent host coding of the new phase before ratify). Under Mode 1: stop and tell the operator to run `atomic-skills:project materialize <phase-id>`. Do not enter degraded mode, do not execute a loose checklist, and do not infer tasks from the descriptor.
...
**Summary (host-thin):** **role banner** at Steps A / C / D.5 / E / F / G / H stating host-thin maestro → **Step H / phase-start package** (phase **objective** + **task list** id+title + **drafted** `businessIntent` **draft**; operator **validate-only** — edit titles/BI allowed, blank form forbidden; no spawn until ratify; **forbid reusing previous writer context**) → one **fresh** phase agent per phase → merge → post-merge `done` → evaluation agent + `evaluationGate` → **decision-review operator PASS** (explicit operator token same turn; host/agent never write `decisionReview` PASS without it; FAIL stamps `failed` and does not advance `currentPhase`) → stamp `decisionReview` → `canRunPhaseDone` preflight → `phase-done` with `review-code --mode=both` → next phase: session handoff **single nextAction** mandatory at the boundary; if successor descriptor-only, automate orchestrates materialize from sidecar + attaches drafted BI into the validation package (operator validates drafted package only — do **not** spawn writer or host-code the new phase until ratify) → plan-end **`external-both`** (codex|grok|claude) + user validation via durable stamp.
