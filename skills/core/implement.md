...
- "I'll flip mode2Enabled back off when I'm done, so pointing Codex at the live tree is only temporary — no state is actually at risk." → The transient window IS the risk: a foreign writer let loose on the live gitignored `.atomic-skills/` state can corrupt it inside that window, and restoring the config afterward un-does nothing (the state is not git-restorable, there's no throwaway boundary to absorb it). "Temporary" and "I'll revert it" do not lift the fence — prove it on a throwaway repo.
...
| "I'll flip the config back after — it's only temporary, no state is at risk" | The transient window IS the risk: a foreign writer can corrupt the gitignored live state inside it, and reverting the config un-does nothing (not git-restorable, no throwaway boundary). Temporary ≠ safe — prove it on a throwaway repo. |
