...
1. **Host-thin pure maestro + product entrypoint ban.** The host session is **host-thin**: it **never** edits product source under `--mode=automate`, and it does **not** run a **product entrypoint** for diagnostics or app execution. **Forbidden** product entrypoint classes on the host (outside **verbatim** task/exit-gate verifiers): `compose`, `build_edl`, app servers, product diagnostics, `npm run dev` / similar app entrypoints. **Allowed host shell actions:** `git merge` / `git status` / `git log`, `assert-automate-gate` (`node scripts/assert-automate-gate.js`), `validate-state`, and task/exit-gate verifier commands **copied verbatim** from `task.verifier` / exit-gate verifier text. Do **not** ban those verbatim verifiers or git-ops merge. On product failure, re-dispatch a code-only **fix agent** — never host product-debug under automate. The host dispatches, sync-waits, merges (git-ops only), re-verifies, reviews, and owns every `done` / `phase-done` / finalize state write.
