...
- "The suite is green, ship it." → Green with `testsCollected: 0` is the false-green. Check the count, not just the exit code.
...
- "Exit 0 — if zero tests ran that's the framework's problem, and CI will catch it anyway." → The 0-count IS the result you must read, not a framework footnote; deferring to CI ships the false-green downstream; and hours already spent are not evidence. PASS needs exit 0 AND `testsCollected > 0`, derived here, now.
...
| "Exit code 0 means it passed" | Not for `kind: test` — exit 0 with 0 tests collected is a vacuous pass. PASS needs exit 0 AND `testsCollected > 0`. |
