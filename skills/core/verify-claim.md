...
1. **Name the claim and its verifier.** State exactly what is asserted done and the deterministic verifier admitted at SPEC time (the task's `verifier:` — `kind shell|test|query`). If the claim has no deterministic verifier, it is not verify-claim's to pass: a `kind: manual` / human-judgement claim routes to the manual-acceptance gate (`{{ASSETS_PATH}}/verifier-exec.md` § `kind: manual`), not here. No verifier at all ⇒ the task should not have been admitted (R-ORCH-23) — surface that, do not invent a pass.
