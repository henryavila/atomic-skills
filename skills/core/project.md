...
{{#if ide.grok}}
   ...
   **Phase scaffold (session todos, F0 pointer only):** contract is `docs/kb/grok-phase-todo-projection.md` (+ `docs/kb/grok-build-compatibility.md` §8). Helper name **reserved for F1**: `scripts/project-session-todos.js` — **not implemented until F1**; do **not** run it as a live pipeline in F0. When wire-up lands, the agent applies the Grok session checklist tool from the helper payload (**Grok-local** phase scaffold only, not `T-00N`). **anti-proc:** while a plan is anchored, process scaffold (`proc:*`) must not compete with the phase scaffold. Do not dump the full SoT procedure into this router.
{{/if}}
