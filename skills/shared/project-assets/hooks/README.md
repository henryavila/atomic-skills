...
- `automate-pen.sh` — PreToolUse for `implement --automate`. No `pen.lock` and no `probe.lock` → exit 0. Either lock → exit 2 for product writes outside the writer worktree and for shells, on Claude Code (`Write`/`Edit`/`MultiEdit`/`Bash`), Codex (`apply_patch`/`shell`), and Grok (`write`/`search_replace`/`run_terminal_command`). `probe.lock` is the startup proof and is not the operational `pen.lock`. `SKIP` does not bypass it.
