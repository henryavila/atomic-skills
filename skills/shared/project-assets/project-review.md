...
- **Optional args:**
  ...
  - `--mode=local|both|both-codex|both-grok|codex|grok|external-both` — forwarded to `review-plan` (default `local`, so the audit stays non-interactive and cheap; `both` adds the host external default envelope). `review` never invents its own mode picker.
...
Only when `--with-code` was passed: run the delegated-write gate from the mutation policy because `review-code` may persist review artifacts and apply fixes. If approved, invoke `atomic-skills:review-code` on the working diff for the resolved target (its Step 0 host-aware picker: local / codex / grok / both* / external-both). Fold its verdict + provider + blocker/critical counts into this report. If the gate is denied or unavailable, print `review-code: skipped (delegated write-capable leg not approved)`. Absent the flag, print `code: skipped (pass --with-code to include)`.
