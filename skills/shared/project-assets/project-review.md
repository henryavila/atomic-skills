...
- `{{BASH_TOOL}} node "$PKG_ROOT/scripts/validate-state.js" .atomic-skills/` — schema validity (FAIL ⇒ a downstream skill / aiDeck will reject the state).
- `{{BASH_TOOL}} node "$PKG_ROOT/scripts/validate-skills.js"` — catalog + skill-body validity.
- `{{BASH_TOOL}} node "$PKG_ROOT/scripts/find-signalless-tasks.js"` — open tasks with neither a `verifier` nor an `outputs[].path` (WARN: undetectable by drift).
- `{{BASH_TOOL}} node "$PKG_ROOT/scripts/find-missing-task-summaries.js"` — tasks missing the authored summary (WARN).
- `{{BASH_TOOL}} node "$PKG_ROOT/scripts/detect-completion.js" --json` (scope with `--project <id>` when the slug is ambiguous) — open entries that look done in the repo (WARN ⇒ recommend `reconcile`).
