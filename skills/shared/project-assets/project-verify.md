...
- **FAIL:** print the validator's errors verbatim. If `--fix` was passed, first run `node "$PKG_ROOT/src/normalize.js" .atomic-skills/`, using the package root already validated by check 0, then re-run `validate-state`. Report what normalization changed. If files still fail after normalization, the failure is structural (not drift) — report it and recommend `migrate <slug>` for legacy files or manual repair.
