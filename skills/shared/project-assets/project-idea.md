...
```sh
node "$PKG_ROOT/scripts/idea-add.js" --title "<título>" --desc "<descrição>"
```
...
```sh
node "$PKG_ROOT/scripts/idea-add.js" --title "<título>" --desc "<descrição refinada>" [--scope "<escopo>"] [--context "<contexto>"]
```
...
```sh
node "$PKG_ROOT/scripts/idea-mark.js" --id <n> --extract            # prints JSON {id, title, date, branch, status, scope?, context?, desc}; no mutation; exit 1 if #n missing/malformed
node "$PKG_ROOT/scripts/idea-mark.js" --id <n> --dest <target-id>   # flips ONLY #n's meta line status:pending → status:triaged→<target-id>; exit 1 if already triaged
```
...
1. **Extract deterministically.** Run with {{BASH_TOOL}}:
   ...
   ```sh
   node "$PKG_ROOT/scripts/idea-mark.js" --id <n> --extract
   ```
...
5. **Materialize, then mark.** Only after the ladder mutation is applied and the target exists — a task id, initiative slug, plan slug, or parked item — run with {{BASH_TOOL}}:
   ...
   ```sh
   node "$PKG_ROOT/scripts/idea-mark.js" --id <n> --dest <target-id>
   ```
