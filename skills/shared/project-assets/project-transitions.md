...
1b. **Automate plan-end + user-validation HARD-BLOCK (plans only — design D7/D12):** when the target is a **plan** and `isAutomateActive` is true for that plan (`src/implement-mode.js` — plan `executionMode: automate` stamp / CLI automate without clear), re-check the same machine gates finalize uses. Soft “point the operator at finalize” is **not** an archive success path.
   - Load frontmatter `planEndReview` (receipt) and `userValidatedAt`.
   - Call `automatePlanEndGatesOk({ automateActive: true, receipt: plan.planEndReview, userValidatedAt: plan.userValidatedAt })` from `src/plan-end-review.js` (single definition — do not re-derive).
   ...
   - When `gates.ok === true`, or when automate is **not** active, continue. Non-automate plan archives are unchanged by this step.
