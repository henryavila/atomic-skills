...
0. **Preflight first (HARD — zero side effects on block).** Call `preflightPhaseDone` (or `classifyLifecycleOrder({ command: 'phase-done', stage: 'preflight', ... })`) from `scripts/lifecycle-order-guard.js` with the parsed initiative + parent plan slice:
   ...
   - **Automate decision-review order (HARD when durable stamp):** under `executionMode: automate`, `canRunPhaseDone` / `decisionReviewAllowsPhaseDone` also require `phases[<this>].decisionReview` with **operator PASS** (`status: passed` + `verifiedAt`) on the phase decision log **before** phase advance. Callers must pass `decisionReview` into `canRunPhaseDone` (and `assert-automate-gate --gate phase-done` reads it from the phase slice). **Agents never stamp decisionReview PASS** — only an explicit operator token in the same turn (e.g. "decision-review PASS") authorizes the stamp. Missing / pending / failed → **STOP**; do not advance `currentPhase`. Non-automate plans skip this check (field optional).
