...
**Completion-series authority.** Before projecting burnup/SPI, consumers canonicalize the append-only completion ledger by `idempotencyKey`. A repeated key is accepted only when every repeated completion has the same logical payload and exactly one reconciliation tombstone authenticates the canonical and ordered duplicate digests; it then contributes once. A contradictory or unneutralized duplicate fails the projection instead of silently selecting the first line.
