...
6. **Review gate** — run `atomic-skills:review-code` on the phase diff. This step is mandatory unless `--skip-review` is passed explicitly.
   ...
   - **CROSS-MODEL REVIEW tracking integration**: before archiving, check `.atomic-skills/status/last-review.json`. If `lastReviewedCommit` ≠ HEAD, announce: "Phase `<id>` is closing with `<N>` commits / `<L>` lines un-reviewed since last CROSS-MODEL REVIEW. Run cross-model review against `<lastReviewedCommit>..HEAD` before archiving? (y/N)". On `y`: invoke `review-due` (`{{ASSETS_PATH}}/project-drift.md` — host external default, not Codex-only). Apply blocker/critical fixes, then proceed.
