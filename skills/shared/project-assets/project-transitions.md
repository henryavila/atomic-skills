...
6. **Review gate** — run `atomic-skills:review-code` on the phase diff. This step is mandatory unless `--skip-review` is passed explicitly.
   ...
   - **Pick the mode via `phaseReviewMode` (single definition — `src/phase-review-mode.js`).** Compute inputs, then call `phaseReviewMode({ automateActive, destructive, explicitOverride })` — do **not** re-derive the ladder in prose elsewhere:
     ...
     2. Compute `review-code`'s `DESTRUCTIVE` signal over this range (the deterministic heuristic in `skills/core/review-code.md` → *Destructive-diff signal*: any whole non-test file deleted, any schema/data drop token, or deletion-dominated churn) → `destructive: true|false`.
     3. Map user flags: `--skip-review` → `explicitOverride: 'skip'`; an explicit recorded `--mode=local` / `--mode=both` → that token. **`skip` is the only full skip; `local` is a downgrade** (still runs local review), never a silent full skip.
     4. Result semantics (encoded only in the helper): explicit override wins; else **when `automateActive` → always `both`** regardless of `destructive`; else non-automate DESTRUCTIVE ladder unchanged — `destructive` → `both`, otherwise `local`. A user may still downgrade a destructive (or automate) phase to `--mode=local`, but only as an explicit, recorded override (note it in the self-review block alongside the review outcome) — never by default.
   - When the helper returns `skip`: do **not** run `review-code`; record skip (next bullet). When it returns `local` or `both`: run `atomic-skills:review-code <range> --mode=<local|both>`. The convergence rule (plateau detection, `--max-iterations`) applies automatically.
...
1b. **Automate plan-end + user-validation HARD-BLOCK (plans only — design D7/D12):** when the target is a **plan** and **`isDurableAutomateActive({ planExecutionMode: plan.executionMode })`** is true (`src/plan-end-review.js` — stamp `executionMode: automate`; **not** session `isAutomateActive` alone), re-check the same machine gates finalize uses. Soft “point the operator at finalize” is **not** an archive success path.
   ...
   - If `gates.ok === false` → **HARD-FAIL archive**: print which sub-gate failed (`planEndReviewOk` and/or `userValidationOk`), leave status untouched, do **not** move files, do **not** run fork-resume or teardown. Resume path:
     - missing / failed plan-end receipt → re-run plan-end `review-code --mode=external-both` (or record `--skip-plan-end-review` with **non-empty** reason — see `{{ASSETS_PATH}}/project-finalize.md` Step 1.7.3) and stamp `planEndReview` + `## Reviews` link under `.atomic-skills/reviews/`;
