...
3. For each criterion (status === `pending`) → apply the **Verifier execution patterns** below. Criteria already `deferred` stay `deferred` and are not reinterpreted as passing evidence.
4. After iterating: report status summary. Continue to advance only when every criterion is `met` OR was explicitly `deferred` with a `deferredReason`. A `pending` criterion blocks phase closure.
5. If any criterion is still `pending` after iteration: ask "Some gates remain pending. Defer the remaining gates and mark phase done anyway? (y/N)". On `n`, leave initiative in `active` state and stop. On `y`, document the override: for each remaining pending criterion, set `status: deferred` and `deferredReason`, then proceed to step 6. Never convert `pending` or `deferred` gates to `met`; `met` means verifier-backed or manual-acknowledged evidence per `verifier-exec.md`.
6. **Review gate** — run `atomic-skills:review-code` on the phase diff. This step is mandatory unless `--skip-review` is passed explicitly.
   ...
   - **Record the review gate on the phase descriptor (G2 — GATE-R3 precondition).** First build the candidate `phases[<this>].reviewGate` in memory: on a clean review, `{ status: passed, at: <reviewed HEAD sha>, mode: <local|both>, reviewFile: <path if written>, verifiedAt: <now> }`; on `--skip-review`, `{ status: skipped, reason: <why>, verifiedAt: <now> }`. Immediately before the first phase-close write, re-read the plan and initiative, set `currentHead` from `git rev-parse HEAD`, set `worktreeDirty` from whether `git status --porcelain` is non-empty, and call `classifyLifecycleOrder({ command: 'phase-done', phase: <fresh initiative>, tasks: <fresh tasks>, exitGates: <fresh authoritative gates>, reviewGate: <candidate>, currentHead, worktreeDirty })` from `scripts/lifecycle-order-guard.js`. If it returns `blocked`, print `reason` + `recommendedCommand` and STOP without writing review metadata, lessons, archive state, or successor state. Only an allowed result may write the candidate gate and continue. This makes a review SHA stale as soon as HEAD changes and blocks unreviewed worktree bytes. The written block remains the durable counterpart to the prose self-review: `validate-state` GATE-R3 HARD-FAILS a `done` phase whose claim lacks its `at`/`reason` anchor. **Do not advance to step 7/8 until this commit guard allows and `reviewGate` is recorded.**
...
8. On the user's accept of an advance:
   - **Propagate completion to the initiative** (BEFORE archiving):
     ...
     b. Mirror initiative `exitGates[]` from the authoritative phase criteria by `id`. Never convert `pending` or `deferred` gates to `met`. If the matching phase criterion is `met`, set the initiative gate `status: met`, copy `metAt`/`evidence` when present, or stamp `metAt: <now>` when absent. If the matching phase criterion is `deferred`, set the initiative gate `status: deferred`, copy `deferredReason` and any failed/attempted `evidence`, and clear `metAt` if present. If the matching phase criterion is still `pending`, abort before archiving and return to steps 3–5; this is a gate leak. If an initiative-only gate has no matching phase criterion, preserve its current status and never coerce it to `met`.
     c. Set initiative `status: done`, `lastUpdated: <now>`, `nextAction: null`.
     d. Recompute the initiative's dashboard rollups (`tasksDone`/`tasksTotal`/`gatesMet`/`gatesTotal`; now all tasks done and gates are either met or explicitly deferred) + per-gate `verifierLabel`/`evidenceSummary` by running `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/refresh-state.js"` (rollups + focus markers + the `focus.json` digest), then save the initiative file.
