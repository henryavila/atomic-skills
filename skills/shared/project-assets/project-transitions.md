...
3. Capture the current HEAD fingerprint: `fingerprint = $(git rev-parse HEAD)`. This value anchors gate evidence and is re-checked by the commit guard.
4. For each criterion with `status === pending` → apply the **Verifier execution patterns** below and write verifier-backed evidence. Never convert `pending` or `deferred` gates to `met` without a real pass. Criteria already `deferred` stay `deferred` and are **not** reinterpreted as passing evidence — they block terminal close.
...
6. **Review gate** — run `atomic-skills:review-code` on the phase diff. This step is mandatory unless `--skip-review` is passed explicitly.
   ...
   - Apply blocker/critical findings. If fixes are committed, the review range and HEAD fingerprint shift — **re-run exit-gate verifiers against the new HEAD** and refresh `fingerprint` before the commit guard (stale evidence must not close).
   - **CROSS-MODEL REVIEW tracking integration**: before archiving, check `.atomic-skills/status/last-review.json`. If `lastReviewedCommit` ≠ HEAD, announce: "Phase `<id>` is closing with `<N>` commits / `<L>` lines un-reviewed since last CROSS-MODEL REVIEW. Run cross-model review against `<lastReviewedCommit>..HEAD` before archiving? (y/N)". On `y`: invoke `review-due` (`{{ASSETS_PATH}}/project-drift.md` — host external default, not Codex-only). Apply blocker/critical fixes, re-verify gates against the new HEAD, then proceed.
   ...
   - **Record the review gate on the phase descriptor (G2 — GATE-R3 precondition).** Before the phase can be set `done` (step 9), write `phases[<this>].reviewGate` on the parent `plan.md`: on a clean review, `{ status: passed, at: <reviewed HEAD sha>, mode: <local|both>, reviewFile: <path if written>, verifiedAt: <now> }`; on `--skip-review`, `{ status: skipped, reason: <why>, verifiedAt: <now> }`. This is the durable, machine-checkable counterpart to the prose self-review block: `validate-state` GATE-R3 HARD-FAILS a `done` phase whose `reviewGate` claims `passed` without an `at` sha, or `skipped` without a `reason`. **Do not advance to step 8/9 until `reviewGate` is recorded** — a phase reaches `done` only *after* its review gate is honestly stamped, never before (this is the "never `done` before review" precondition: until then the phase stays `active`/verify-pending).
...
7. **Commit guard (HARD — re-read before any terminal write).** Fresh-read the initiative + plan from disk. Call `commitGuardPhaseDone` (or `classifyLifecycleOrder({ command: 'phase-done', stage: 'commit', ... })` / `decidePhaseDoneTerminal({ ... })`) with:
   ...
   - current `exitGates[]` / phase criteria (every gate must be `status: met` — deferred/pending/failed/declined block with codes `phase-done-gate-deferred` / `phase-done-open-gate`),
   - recorded `reviewGate` + `lessonsState`,
   - `fingerprint: <current HEAD>` and `expectedFingerprint: <fingerprint captured when gate evidence was produced>` (mismatch ⇒ `phase-done-fingerprint-stale`; re-run verifiers).
