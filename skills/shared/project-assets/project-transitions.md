...
5b. **Grok session-todo projection (session-local only; never close authority).** After `refresh-state` succeeds, run `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/project-session-todos.js" --json` and apply the resulting payload with the Grok session checklist tool (`todo_write`) when operating on Grok (see `docs/kb/grok-phase-todo-projection.md`). SoT order is mutate → `refresh-state` → `project-session-todos` → `todo_write`. Session todos are a **projection only** — never close authority relative to plan/initiative YAML; never mark a phase session todo `completed` unless that phase's durable status is already `done` or `archived`. Do **not** treat a checked session todo as task evidence or as a substitute for this flow's verifier gate.
...
The **only detection-drift-triggered completion-mutation path** (Spec 1, Component B). Detection (`detect-completion`, `status`, `verify`) is report-only; `done` remains the **closure authority** for task state (and verifier-backed criterion close still runs the Verifier execution patterns / GATE-R2). `reconcile` is where the user disposes of each *detected* candidate — it is not a second close authority and never silent-auto-closes. Subject to the standard pre-mutation gates (migration check, reconciliation gate). Path is always **signal → ask → verifier run or manual ack** (or Still open / Skip). It NEVER fabricates a close: the detection signal (a changed deliverable) and the close authority (a passing verifier *or* an explicit human ack via `done` / manual criterion ack) stay separate.

1. Run the deterministic detector: `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/detect-completion.js" --json` (add `--project <id>` when more than one project holds the resolved plan-slug; `--slug`/`--plan` to widen within the project). It returns `candidates[]`, each carrying `kind` (`task`|`criterion`), `id`, the resolved `initiativePath` (the **safe write target** — never a bare slug), `evidence` (`output-exists`|`commit-ref`), `paths`/`commit`, and `hasVerifier` + `verifier`. The detector is pure-read: it never mutates state and never runs a verifier.
...
3. For each candidate (batch the **oldest 4 first**, mirroring the reconciliation gate), present a {{ASK_USER_QUESTION_TOOL}} whose options are **verifier-aware**:
   ...
   - **`Still open`** → acknowledge without closing (no status change). Reset **only schema-supported signal anchors** so the same candidate does not re-surface immediately:
     - **task** → bump that task's `lastUpdated` to now (tasks own this field).
     - **criterion** → bump the **initiative's** `lastUpdated` to now. ExitCriterion is `additionalProperties: false` and has **no** `lastUpdated`; never write one onto a criterion (that would invalidate schema). Detection anchors pending criteria on the initiative `lastUpdated`/`started`, so the initiative stamp is the supported clock for gates.
   - **`Skip`** → no change.
4. After applying dispositions, recompute the initiative's dashboard rollups by running `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/refresh-state.js"` (rollups + focus markers + the `focus.json` digest, one pass) and save. If closing the last open task of a phase initiative, the `done` flow's auto-transition fires the `phase-done` offer at the right time — that loop-close is the whole point of making this moment reliably reachable.
5. **Grok session-todo projection (session-local only; never close authority).** After `refresh-state`, run `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/project-session-todos.js" --json` and apply the payload with the Grok session checklist tool (`todo_write`) when on Grok. Same SoT order as `done`: mutate → `refresh-state` → `project-session-todos` → `todo_write`. Never mark a phase session todo `completed` unless that phase's durable status is already `done` or `archived`; never close durable state because a session todo is checked.

This is the pause-point applied to completion drift, as an explicit verb — so `status`/`verify` keep their read-only semantics, `done` stays the task closure authority, and the user is never trapped (every candidate has a valid action for its verifier state; never silent done).

## `phase-done`

Invoked when the active initiative is the phase initiative of an active plan AND all its tasks are **already** closed via `done` (each with its own verifier + `task-done` event). Pure preflight runs first; exit-gate evidence and review run only after preflight allows; a final commit guard re-reads state before any terminal write. **No bulk-close of open tasks. No defer/skip of exit gates as a terminal path.** A gate that is `pending`, `failed`, `declined`, `deferred`, or missing current evidence keeps the phase `active` or `paused` with **zero** terminal transition (no close write, no completion event, no archive, no advance commit).

### Stage A — pure preflight (before gates / review)

0. **Preflight first (HARD — zero side effects on block).** Call `preflightPhaseDone` (or `classifyLifecycleOrder({ command: 'phase-done', stage: 'preflight', ... })`) from `scripts/lifecycle-order-guard.js` with the parsed initiative + parent plan slice:
   - **identity** — `parentPlan` + `phaseId` present; when a plan is loaded, the phase id/slug must exist on `plan.phases[]`.
   - **DAG** — when a plan is loaded, `validatePhaseDag(plan)` must be ok (unknown deps / self-loops / cycles block).
   - **tasks** — a parsed `tasks[]` array is required and every task must already be `status: done`. An open task returns `phase-done-open-task` and recommends `done <id>`.
   - **Automate evaluation order (HARD when durable stamp):** `preflightPhaseDone` enforces this via `phaseEvaluationAllowsClose` / code `phase-done-evaluation-open` when `plan.executionMode` is automate (or `isDurableAutomateActive`). Callers must pass `evaluationGate` from `phases[<this>].evaluationGate`. If blocked → **STOP** with reason (run evaluation agent first, or record skip/disposition). Do **not** run exit-gate verifiers or review-code until evaluation allows close. Non-automate plans skip this check. `validate-state` GATE-R4 also hard-fails automate plans whose done phases lack an honest evaluationGate.
   - **Delivery audit (HARD — Mode-1 + pure-maestro; never skippable under durable automate):** before terminal phase-done, run `atomic-skills:audit-delivery` for the phase, write the report under `.atomic-skills/reviews/`, and stamp `phases[<this>].deliveryAuditGate` via `buildDeliveryAuditGate` (`status: passed`, non-empty `reportPath`, `verdict: CLOSED|PARTIAL`, `verifiedAt`). **No** `operatorSkip`, **no** `status: skipped`, **no** OPEN as passed. Plan-end `intentVsDelivered` is **not** a substitute. `preflightPhaseDone` / `commitGuardPhaseDone` enforce via `deliveryAuditAllowsClose` (code `phase-done-delivery-audit-open`). Layer-2 `assert-automate-gate --gate phase-done` also content-authenticates the report file (exists + content floor + stamp verdict matches body). `validate-state` GATE-R4 hard-fails automate done phases missing an honest deliveryAuditGate.
   - **Layer-2 assert (automate pure-maestro / durable stamp):** before continuing Stage A/B after evaluation allows close, run `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/assert-automate-gate.js" --plan <slug> --gate phase-done` (wraps `canRunPhaseDone` + deliveryAudit authenticity). **Non-zero exit ⇒ STOP** with the printed `blocked:` reason — same fail-closed semantics as preflight; do not run exit-gate verifiers or review-code. Prefer this CLI call site from `implement` pure-maestro Step G; project skill may still call `preflightPhaseDone` directly.
   - **Maestro cursor step G (automate stamp):** under durable `executionMode: automate`, the Layer-2.5 cursor (`.atomic-skills/status/automate/<slug>.json`) must allow `--gate phase-done` (step **G**). After a **successful** terminal phase-done under automate, set **`awaiting-operator-advance`** via `recordPhaseDonePause` / `recordPhaseDonePauseFile` in `src/maestro-cursor.js` — do **not** auto-materialize the next phase, do **not** auto-finalize, do **not** chain F_n→F_n+1 spawn. Next-phase re-entry requires explicit **`clearContinue`** with continue token `operator-continue` (or `operatorContinue: true`) → step **H**. Generic ok / bare phase-done y is **not** that token when ratify rules apply. **Non-automate phase-done is unchanged** — do not call `recordPhaseDonePause` without the automate stamp.

1. Load the active initiative. Verify it has `parentPlan` + `phaseId` set (same identity as preflight). Else abort.
2. Load the parent plan (resolve `<parentPlan>` nested-first: `projects/<project-id>/<parentPlan>/plan.md`, legacy `.atomic-skills/plans/<parentPlan>.md`). Locate the phase by `phaseId`. Read `phase.exitGate.criteria` AND `initiative.exitGates` (combine; phase-level criteria are authoritative for the gate). Re-run preflight with the loaded plan slice if it was not already passed with that slice.

### Stage B — produce evidence (only after green preflight)

3. Capture the current HEAD fingerprint: `fingerprint = $(git rev-parse HEAD)`. This value anchors gate evidence (`evidence.verifiedCommit`) and is re-checked by the commit guard. **A HEAD change after this capture invalidates prior evidence** — re-run verifiers before the commit guard.
4. For each criterion with `status === pending` → apply the **Verifier execution patterns** (`{{ASSETS_PATH}}/verifier-exec.md`) and write verifier-backed evidence **including `verifiedCommit: <fingerprint>`** (the SHA at run time). Never convert `pending` or `deferred` gates to `met` without a real pass. Criteria already `deferred` stay `deferred` and are **not** reinterpreted as passing evidence — they block terminal close. Do not accept an arbitrary non-SHA string as `verifiedCommit` (`validate-state` + schema `gitSha` reject it).
5. After iterating: report status summary. **Continue only when every criterion is `met` with current evidence.** A `pending`, `failed`, `declined`, or `deferred` criterion blocks phase closure. **Do not offer defer/skip as a terminal path.** If any gate is not `met`: leave the initiative `active` (or `paused` if the operator chooses to pause), record any partial evidence, and **STOP** with zero terminal transition. The only exits without every gate green are: keep working, or pause the phase — never close.
...
6. **Review gate** — run `atomic-skills:review-code` on the phase diff. Under
   ...
   - **Pick the mode via `phaseReviewMode` (single definition — `src/phase-review-mode.js`).** Call `phaseReviewMode({ planExecutionMode, automateActive, destructive, explicitOverride })` — durable stamp via `isDurableAutomateActive` (session clear does **not** open skip):
     1. Pass `planExecutionMode` from plan frontmatter (stamp-first).
     2. Compute `review-code`'s `DESTRUCTIVE` signal over this range → `destructive: true|false`.
     3. Outside durable automate only: map `--skip-review` → `explicitOverride: 'skip'`; `--mode=local|both` → that token.
     4. Under durable automate the helper **always** returns `both` (local/skip ignored). Outside automate: override wins, else destructive → `both`, else `local`.
   - When the helper returns `skip` (non-automate only): do **not** run `review-code`; record skip. When it returns `local` or `both`: run `atomic-skills:review-code <range> --mode=<local|both>`.
   - Apply blocker/critical findings. If fixes are committed, the review range and HEAD fingerprint shift — **prior gate evidence is invalidated**. **Re-run exit-gate verifiers against the new HEAD**, stamp each criterion's `evidence.verifiedCommit` to the new SHA, and refresh `fingerprint` before the commit guard (stale `verifiedCommit` / fingerprint ⇒ `phase-done-fingerprint-stale`; do not reuse evidence from before the review commit).
   - **CROSS-MODEL REVIEW tracking integration**: before archiving, check `.atomic-skills/status/last-review.json`. If `lastReviewedCommit` ≠ HEAD, announce: "Phase `<id>` is closing with `<N>` commits / `<L>` lines un-reviewed since last CROSS-MODEL REVIEW. Run cross-model review against `<lastReviewedCommit>..HEAD` before archiving? (y/N)". On `y`: invoke `review-due` (`{{ASSETS_PATH}}/project-drift.md` — host external default, not Codex-only). Apply blocker/critical fixes, re-verify gates against the new HEAD (refresh `verifiedCommit`), then proceed.
   - If `--skip-review` was passed: record `CROSS-MODEL REVIEW: SKIPPED at phase-done (user requested --skip-review)` in the self-review block (see § "Self-review" below) and proceed.
   - **Record the review gate on the phase descriptor (G2 — GATE-R3 precondition).** Before the phase can be set `done` (step 9), write `phases[<this>].reviewGate` on the parent `plan.md`: on a clean review, `{ status: passed, at: <reviewed HEAD sha — real git SHA>, mode: <local|codex|both>, reviewFile: <path if written>, verifiedAt: <now> }`; on `--skip-review`, `{ status: skipped, reason: <why>, verifiedAt: <now> }`. This is the durable, machine-checkable counterpart to the prose self-review block: `validate-state` GATE-R3 HARD-FAILS a `done` phase whose `reviewGate` claims `passed` without a real `at` git SHA + coherent `mode` (and rejects blank `reviewFile` when the field is present), or `skipped` without a `reason`. Arbitrary non-SHA strings in `at` are rejected. **Do not advance to step 8/9 until `reviewGate` is recorded** — a phase reaches `done` only *after* its review gate is honestly stamped, never before (this is the "never `done` before review" precondition: until then the phase stays `active`/verify-pending).
   - **Distill lessons — part of the DEFINITION of phase-done, not optional (G1, the Spec-2 capture).** A `review-code` finding *generates* learning, but nothing aggregates it where the NEXT phase will read it — so the lesson evaporates unless the user happens to ask (the F0-decommission "15k orphaned emails" lesson was nearly lost exactly this way). After the review gate, **draft the phase's lessons from real failure signals only** — confirmed `review-code` findings (especially a cross-model blocker the local pass missed), reopened/blocked tasks, deferred gates (still open follow-ups — never treated as passed), the phase diff, and user corrections made this phase. For each, write one entry: a `statement` (what went wrong, blameless) + a `corrective` (the locus + what to do next time — Self-Refine actionable shape), `scope: reusable` if it generalizes beyond this phase else `local`, `appliesTo: []` (all future phases) or specific `phaseId`s, `status: open`, `confidence: 2` (ExpeL born-2), `evidence` (link to the review file / self-review), `createdAt`/`validatedAt: <now>`. Then **present a `Proposed lessons:` block and have the user ratify / edit / reject** (the existing ratify-gate; selective + capped + blameless — verify-before-store). Write the ratified set to the per-initiative lessons file `.atomic-skills/projects/<project-id>/<parentPlan>/lessons/<initiative-slug>.md` (create it if absent: `{ schemaVersion, slug: <initiative-slug>, projectId, parentPlan, lessons: [...] }`), then `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/validate-state.js"` that file. A clean phase with no failure signal records **zero lessons explicitly** (note "no lessons distilled — clean phase" in the self-review block; set `lessonsState: none`) — silence ≠ zero; the step is always *answered* (`lessonsState: recorded` or `none`). The phase-start gate (`project-create-initiative.md`) is what surfaces these into the next phase via `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/list-lessons.js" --project <project-id> --plan <parentPlan> --phase <next-phase-id>`.

### Stage D — commit guard + terminal transition

7. **Commit guard (HARD — re-read before any terminal write).** Fresh-read the initiative + plan from disk. Call `commitGuardPhaseDone` (or `classifyLifecycleOrder({ command: 'phase-done', stage: 'commit', ... })` / `decidePhaseDoneTerminal({ ... })`) with:
   - current `tasks[]` (still all `done` — **no bulk-close**; open task ⇒ zero writes/events/commits),
   - current `exitGates[]` / phase criteria (every gate must be `status: met` — deferred/pending/failed/declined block with codes `phase-done-gate-deferred` / `phase-done-open-gate`; each met gate's `evidence.verifiedCommit` must match current HEAD when present),
   - recorded `reviewGate` (`passed` ⇒ real SHA `at` + `mode`; `skipped` ⇒ `reason`) + `lessonsState`,
   - `fingerprint: <current HEAD>` and `expectedFingerprint: <fingerprint / verifiedCommit captured when gate evidence was produced>` (mismatch ⇒ `phase-done-fingerprint-stale`; `reviewGate.at` ≠ HEAD ⇒ `phase-done-review-stale`). **When review applied fixes and mutated HEAD, re-run verifiers first** — the guard refuses to close on pre-review evidence.
   If the commit guard returns `blocked`: print `reason` + `recommendedCommand`, leave phase non-terminal (`active`/`paused`), and **STOP** with zero close write, zero completion event, zero archive, zero advance. Status-edit shortcuts and "direct advance" without a green commit guard are forbidden — `proposeAdvance` is advisory only until this guard allows.

8. **Advance proposal (advisory only until step 9).** Call `src/transition.js`:`proposeAdvance(plan, phaseId)` to decide what's next. The function returns one of:
   ...
   - `{ kind: 'blocked' | 'error', ... }` — do not write terminal status; surface open work / DAG errors.
...
9. On the user's accept of an advance (and only with a green commit guard from step 7):
   - **Automate pause after successful phase-done (HARD when stamp):** when the parent plan has `executionMode: automate`, after the terminal close writes succeed, call `recordPhaseDonePauseFile` (or pure `recordPhaseDonePause` + `writeCursorFile`) so the maestro cursor is **`awaiting-operator-advance`**. Announce that next-phase spawn / pure-maestro Step A is **blocked** until operator continue: `clearContinue` with `continueToken: 'operator-continue'` (constant `OPERATOR_CONTINUE_TOKEN`) → step **H**, then materialize if needed and re-enter `implement`. **Do not** auto-run multi-phase or treat this accept alone as continue under pause. Non-automate: skip this bullet entirely (phase-done advance UX unchanged).
   - **Propagate completion to the initiative** (BEFORE archiving) — **no bulk-close of tasks**:
...
10. On user decline of the advance (or `plan-done` accept without `currentPhase` change) — still only with a green commit guard:
   - **Propagate completion to the initiative** (same steps 9a-d above).
   - Set the parent plan's phase `status: done` and stop without seeding a successor.
   - **Post-close projection** (same shared step as accept path after durable phase done): re-run `refresh-state` → `project-session-todos` → `todo_write` on Grok. Never mark the phase session todo `completed` without durable plan+initiative phase status `done` or `archived`.
...
- **G6 reference-or-strike**: K exit criteria, J met with `evidence:` populated, L still deferred/pending (phase cannot close until met), M unverified-and-flagged.
...
- **CROSS-MODEL REVIEW**: ran via `atomic-skills:project review-due` (provider `<codex|grok>`) at HEAD = `<sha>`, verdict `<v>`, counts `<…>`, file `.atomic-skills/reviews/<…>.md`. / SKIPPED at phase-done per user (`<reason or "no reason given">`).
...
5. On accept:
   - If the matching initiative file is absent (descriptor-only):
     - **Mode 1 / non-automate:** run `atomic-skills:project materialize <phase-id>` as an internal transition caller with the selected active phase id set containing the reopened target (Mode A blank-form BI); do not propose `new initiative` for descriptor-only phases. `materialize` owns the businessIntent/decompose/write/validate flow; this command does not duplicate it or overwrite an existing initiative file.
     - **Automate (`executionMode: automate` / pure-maestro):** do **not** blank-form materialize (Mode A). Either continue only when a continuous package→ratify→Mode B sequence supplies a pre-ratified complete spine, **or refuse** with zero initiative write and set handoff **single nextAction** to present the phase-start package / await ratify (same contract as phase-done successor + `project-materialize.md` Mode B fail-closed). Step H/B of implement pure-maestro owns draft → validate-only → ratify → Mode B materialize.
...
9. Run `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/refresh-state.js"` (rollups + focus markers + `focus.json`), then **Grok session-todo projection**: run `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/project-session-todos.js" --json` and apply via the Grok session checklist tool (`todo_write`) when on Grok. SoT order: mutate → `refresh-state` → `project-session-todos` → `todo_write`. A reopened phase is not `done`/`archived` — do **not** leave its phase session todo `completed`.

## `detect-scope`

Wrap `scripts/detect-scope.js` to suggest a `scope.paths` value based on recent git activity.

1. Run `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/detect-scope.js" --json --branch=<active-branch> --limit=20` via {{BASH_TOOL}}.
...
1b. **Automate plan-end + user-validation HARD-BLOCK (plans only — design D7/D12):** when the target is a **plan** and **`isDurableAutomateActive({ planExecutionMode: plan.executionMode })`** is true (`src/plan-end-review.js` — stamp `executionMode: automate`; **not** session `isAutomateActive` alone), re-check the same machine gates finalize uses. Soft “point the operator at finalize” is **not** an archive success path.
   - Load frontmatter `planEndReview` (receipt) and `userValidatedAt`.
   - Call `automatePlanEndGatesOk({ planExecutionMode: plan.executionMode, receipt: plan.planEndReview, userValidatedAt: plan.userValidatedAt })` from `src/plan-end-review.js` (or `canFinalizeOrArchive` from `src/automate-orchestrator-gates.js` — same predicate; do not re-derive).
   - **Layer-2 assert:** prefer `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/assert-automate-gate.js" --plan <slug> --gate finalize` (exit 0 required). Non-zero ⇒ same HARD-FAIL as `gates.ok === false`.
   - If `gates.ok === false` → **HARD-FAIL archive**: print which sub-gate failed (`planEndReviewOk` and/or `userValidationOk`), leave status untouched, do **not** move files, do **not** run fork-resume or teardown. Resume path:
     - missing / failed plan-end receipt → re-run plan-end `review-code --mode=external-both` and stamp `planEndReview` + `## Reviews` link under `.atomic-skills/reviews/`. **Under durable `executionMode: automate`, `--skip-plan-end-review` is HARD-CLOSED** (`planEndReviewOk(receipt, { forbidSkip: true })` / `automatePlanEndGatesOk.reviewSkipForbidden`) — only a real external-both receipt opens finalize/archive; clear the stamp to leave automate if residual risk must be accepted outside the gate;
     - missing `userValidatedAt` → operator validates implementation + decisions log, then stamp ISO `userValidatedAt`.
   - When `gates.ok === true`, or when durable automate is **not** active (no stamp), continue. Non-automate plan archives are unchanged by this step.
   - **Phase-archive exception:** when `targetKind: 'phase'` / `caller: 'phase-done'` (internal `phases/archive/` move), this step does **not** apply — phase close already ran its own review gate; plan-end gates fire only on whole-plan archive.
...
4. **Initiative archival**:
   ...
   - If the initiative has `parentPlan` and the matching plan phase has `status: done`, verify that the initiative `status` is `done` (not `active`/`pending`). If not, apply the propagation steps from `phase-done` step 9a-d first (assert tasks already `done` — no bulk-close; mirror exitGate `met` statuses without coercing pending/deferred gates to `met`; initiative `status: done` only after commit guard), then continue.
...
5b. Run `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/refresh-state.js"` (rollups + focus markers + `focus.json`), then **Grok session-todo projection**: run `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/project-session-todos.js" --json` and apply via the Grok session checklist tool (`todo_write`) when on Grok. SoT order: mutate → `refresh-state` → `project-session-todos` → `todo_write`. Archived phases may project as session todo `completed` only because durable status is already `archived` — never the reverse.
...
2. **Plan switch**:
   ...
   - Resolve the target plan's `currentPhase` (or its `active` phase descriptor when `currentPhase` is absent). If the matching initiative file exists, reuse it: set that initiative to `status: active`, refresh `lastUpdated`, and do not overwrite it. If the initiative file is absent (descriptor-only):
     - **Mode 1 / non-automate:** run `atomic-skills:project materialize <phase-id>` as an internal transition caller with the selected active phase id set containing the target active/current phase (Mode A blank-form BI), after the old active plan/initiative has been paused/demoted and before reporting the switch complete; do not propose `new initiative` for descriptor-only phases.
     - **Automate (`executionMode: automate` / pure-maestro):** do **not** blank-form materialize (Mode A). After the plan/pointer is active on the descriptor-only target, either continue only on continuous package→ratify→Mode B with a pre-ratified spine, **or refuse** materialize with zero initiative write and set handoff **single nextAction** to present package / await ratify (`project-materialize.md` Mode B fail-closed; Step H/B owns the ritual).
   ...
   - **Grok session-todo projection (session-local only; never close authority).** After `refresh-state`, run `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/project-session-todos.js" --json` and apply via the Grok session checklist tool (`todo_write`) when on Grok (reseed with `merge: false` when pickFocus has a winner). SoT order: mutate → `refresh-state` → `project-session-todos` → `todo_write`. Never mark a phase session todo `completed` unless that phase's durable status is already `done` or `archived`.
3. **Initiative switch**:
   ...
   - If the matching initiative file is absent and the active plan descriptor has the target phase (descriptor-only), set the target phase descriptor to `status: active`, set `currentPhase` to that phase id, then:
     - **Mode 1 / non-automate:** run `atomic-skills:project materialize <phase-id>` as an internal transition caller with the selected active phase id set containing the target phase (Mode A). Do this after any old active initiative is paused and before reporting the switch complete; do not propose `new initiative` for descriptor-only phases.
     - **Automate (`executionMode: automate` / pure-maestro):** do **not** blank-form materialize (Mode A). Same package→ratify→Mode B or refuse + **single nextAction** to present package / await ratify as plan-switch above.
   ...
   - **Grok session-todo projection:** same as plan-switch — after `refresh-state`, run `project-session-todos` and apply via `todo_write` on Grok; never complete a phase todo without durable `done`/`archived`.
4. Announce.
