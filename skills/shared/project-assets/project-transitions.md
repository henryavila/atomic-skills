...
0. **Preflight first (HARD — zero side effects on block).** Call `preflightPhaseDone` (or `classifyLifecycleOrder({ command: 'phase-done', stage: 'preflight', ... })`) from `scripts/lifecycle-order-guard.js` with the parsed initiative + parent plan slice:
   ...
   - **Automate decision-review order (HARD when durable stamp — same preflight):** `preflightPhaseDone` **also** enforces operator decision-review via `decisionReviewAllowsPhaseDone` / code `phase-done-decision-review-open` under the same durable automate stamp. Callers must pass `decisionReview` from `phases[<this>].decisionReview` with **operator PASS** (`status: passed` + `verifiedAt`). Missing / pending / failed / passed-without-`verifiedAt` → **STOP** with zero side effects (no exit-gate verifiers, review-code, or phase advance). This is **not** optional CLI-only: it is wired into `preflightPhaseDone` / `commitGuardPhaseDone` (and into `canRunPhaseDone` / `assert-automate-gate --gate phase-done` for the same stamp). **Agents never stamp decisionReview PASS** — only an explicit operator token in the same turn (e.g. "decision-review PASS") authorizes the stamp. Non-automate plans skip this check (field optional). `validate-state` GATE-R4 also hard-fails automate plans whose done phases lack an honest decisionReview (status=passed + verifiedAt).
   If preflight returns `blocked`: print `reason` + `recommendedCommand`, leave the initiative untouched, and **STOP**. Do **not** run exit-gate verifiers, review-code, distill lessons, emit completion events, archive, flip status, call `proposeAdvance` as a write path, or commit. Use `decidePhaseDoneTerminal({ stage: 'preflight', ... })` when a machine-checkable empty effect set is needed — blocked ⇒ `writes: []`, `events: []`, `commits: []`, `terminal: false`.
...
9. On the user's accept of an advance (and only with a green commit guard from step 7):
   - **Propagate completion to the initiative** (BEFORE archiving) — **no bulk-close of tasks**:
     ...
     e. **Grok session-todo projection (session-local only; never close authority).** After `refresh-state`, run `node "$(cat "$HOME/.atomic-skills/package-root" 2>/dev/null || echo .)/scripts/project-session-todos.js" --json` and apply the payload with the Grok session checklist tool (`todo_write`) when on Grok. SoT order: mutate → `refresh-state` → `project-session-todos` → `todo_write`. Only after this phase's durable status is `done` (or later `archived`) may its phase session todo be `completed` — never mark a phase todo completed to *cause* close, and never without phase status `done` or `archived`.
