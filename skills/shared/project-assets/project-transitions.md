...
3. Choose one immutable `closedAt: <now>`. Build one close candidate containing task `status: done`, `closedAt`/`lastUpdated`, the passing evidence, the new `nextAction`, and the complete `## Session handoff` fields. Do not derive or assert the idempotency key at the caller: the coordinator reloads the authoritative task and assigns `completionGeneration = previous + 1` (or `1` on the first close), then derives the branch-stable key from that generation. Persist `tasks[].completionProvenance` with that exact `nextAction`, handoff, and any frozen task `actuals`; this schema-backed block is the markerless recovery authority after the transaction marker is cleared. **Advance `nextAction` (C-5)** to exactly one verified imperative: the next unblocked task, the unblock path when only blocked tasks remain, `Run \`phase-done\`` when an in-plan phase has no open tasks, or `Run \`archive <slug>\`` for a standalone initiative.
4. Execute `executeDoneTransaction(...)` from `scripts/done-transaction.js`. The coordinator first acquires the plan-wide `plan-state` lock scoped to `projectId/planSlug`, then the shared `phase-state` lock scoped to `projectId/planSlug/phaseId`; both scopes deliberately exclude `taskId` and `closedAt`. `effects.loadInitiative` must reload the authoritative project/plan/phase initiative inside those locks. Task close and phase close mutate the same initiative, while materialization and parallel phase closes can replace the shared plan, so every lifecycle writer follows the fixed order `plan-state` → `phase-state` → materialization/completion internals. The coordinator verifies that the close scope and unique task belong to that fresh initiative, persists the immutable close bundle plus digest in a durable recovery marker, then in order: persists the complete close bundle including `tasks[].completionGeneration` and `tasks[].completionProvenance`; runs `refresh-state`; calls `ensureCompletion(root, { event: 'task-done', projectId, planSlug, phaseId, taskId, generation, idempotencyKey })`; finds or creates the explicit-path checkpoint containing initiative state, handoff, derived state, and analytics; confirms the task-owned worktree is clean; finally removes the marker. Never append the event before durable close state.
5. On retry, read the marker and resume only the unfinished stage against fresh authoritative state. A changed close bundle/evidence/handoff fails closed; `ensureCompletion` returns the existing immutable event for the same generation key; a stored checkpoint is reauthenticated by `findCheckpoint` before cleanup. A concurrent branch close of the same authoritative generation derives the same key even when its timestamp differs. A real reopen retains `completionGeneration`, and the next close increments it so both generations remain auditable. If either artifact is missing after state persistence, the coordinator recreates a `state-persisted` recovery marker and repairs that artifact under the original identity and frozen task actuals; missing historical completion provenance fails closed instead of consulting a newer dispatch record. It never reports a null event/checkpoint or creates a second logical close. The marker remains at `.atomic-skills/status/done-transactions/<sha256>.json` after any failed boundary and is removed only after the complete checkpoint is authenticated and proven clean. A terminal (`done`/`archived`) initiative is immutable and rejects every new task close.
   Coordinator-owned locks, recovery markers and the completion ledger are confined component-by-component beneath the canonical repository root. A symbolic link in `.atomic-skills`, `status`, `analytics`, or their transaction subdirectories fails closed before any external write. Durable publication fsyncs file bytes and, where the platform supports directory descriptors, the containing directory; Windows skips only the unsupported directory-fsync step.
...
8. On the user's accept of an advance:
   - Call `executePhaseDoneTransaction(...)` from `scripts/phase-done-transaction.js` exactly once with the guarded candidate bundle. Every bullet below is implemented **inside** that coordinator's named effects; none is an independent write path:
     - `effects.loadState` reloads the authoritative plan, unique descriptor, initiative, tasks, and gate mirrors after the coordinator acquires `plan-state(projectId/planSlug)` and then `phase-state(projectId/planSlug/phaseId)`. Both exclude `closedAt`; a concurrent later timestamp therefore reuses the terminal phase instead of creating another close, and different phases cannot publish stale whole-plan snapshots concurrently.
     - `effects.findCommit` authenticates an existing close checkpoint for this derived key.
     - `effects.commit` atomically persists the mirrored gates, review receipt, lessons, the coordinator-assigned `completionGeneration` on both phase mirrors, terminal plan/initiative state, archive move, handoff and selected plan advance using explicit paths, then returns the full `closeSha`.
- Compute aggregate `actuals` before entering the transaction and pass them as
  `input.actuals`. The coordinator freezes them in its recovery marker before
  commit; `effects.emit` and the coordinator's mandatory `ensureCompletion`
  authentication both use that marker-owned value for the one logical
  `phase-done` record.
     - `effects.materializeSuccessor` publishes a descriptor-only successor through `materializeState({ prerequisiteCloseSha: closeSha, successor: marker.successor, planLockCapability, ... })`. The fourth effect argument supplies the explicit active `planLockCapability`; pass it through rather than reacquiring or inventing reentrancy. Materialization binds the persisted manifest to the canonical plan/initiative paths, candidate phase/plan identity, and SHA-256 of both exact candidate byte streams, then persists the manifest and digest in its recovery marker. The effect returns the `materializeState` result unchanged. The coordinator validates its `publication` evidence against `marker.successor` before advancing from `emitted`; missing, no-op, forged-shape, changed or omitted evidence leaves the recovery marker live. With no accepted successor, omit both the manifest and effect.
     - `effects.assertClean` proves that no task-owned bytes remain outside the durable recovery marker.
     A retry reuses the marker and authenticated `closeSha`; it never reruns the new-close commit guard after the close commit and never performs a terminal effect outside the coordinator. When fresh authoritative state is already terminal and no marker remains, the coordinator first revalidates the close guard against the immutable review anchor, requires exactly one canonical `phase-done` event for the phase, authenticates that event's `closeSha` through `effects.findCommit`, and returns through a read-only path. It never trusts or replays a caller-supplied successor manifest: marker cleanup already proves any accepted successor publication and the final clean check completed. Terminal mirrors alone are never proof of completion.
   - **Propagate completion to the initiative** (inside `effects.commit`, BEFORE archiving):
     a. Assert again that all tasks were already closed individually by `done <task-id>` and that each close emitted its own completion event. `phase-done` never mutates an open task. Emit no `task-done` completion events here. Compute `actuals` with `computePhaseActuals(phaseStarted, { cwd: root, sinceCommit: phaseStartedCommit })`, where `phaseStartedCommit` is the initiative's immutable `startedCommit` and `phaseStarted` is its phase timestamp fallback, and pass the result as `input.actuals`. The coordinator freezes that value before commit and authenticates exactly one `phase-done` completion event with the derived `idempotencyKey` and `closeSha`, carrying the phase's aggregate actuals once. Omit unavailable actuals; never invent them.
...
**Completion-series authority.** Before projecting burnup/SPI, consumers canonicalize the append-only completion ledger by `idempotencyKey`. New task/phase keys bind to the authoritative `completionGeneration`, not branch-local timestamps; legacy timestamp keys remain readable. A repeated key is accepted only when every repeated completion has the same generation payload and a stable, order-independent reconciliation tombstone authenticates the canonical and sorted duplicate digests; repeated physical copies of that exact tombstone collapse to one logical marker after a union merge. A conflicting tombstone payload or unneutralized completion duplicate fails the projection. Distinct generations remain distinct earned events.
...
5. On accept:
   ...
   - If the matching initiative file exists, reuse it:
     ...
     - Set all `tasks[].status = 'pending'`; clear `tasks[].closedAt`; refresh `tasks[].lastUpdated = <now>`. Retain every task's `completionGeneration` and the phase mirrors' generation so the next legitimate close increments instead of aliasing the prior close.
