...
## Step 1.5 — The typed resolution ALLOWLIST (the proof) — `scripts/consolidation-resolve.js`

`classifyConflictPath` is **pure, never-throws, FAIL-CLOSED**. The table is a strict
**allowlist**; a path not provably mechanical resolves to **`eject`** (route to human) —
never a silent mechanical sweep. A mis-classification would produce a tree that
compiles+tests green while having dropped a side's real work, which the build+test floor
cannot catch — the allowlist is what prevents that.

| Class | Example | Policy |
|---|---|---|
| runtime-regenerable | `.atomic-skills/focus.json` | take-delete (re-emit from source) |
| ledger-append | `reviews/INDEX.md`, `ideas.md`, `dispatch-log.json` | union (lossless) |
| config-union | `.gitignore`, `.gitattributes` | union |
| pointwise-state | `last-review.json`, `last-session.json` | last-writer-wins (in-file timestamp) |
| narrative-index | `PROJECT-STATUS.md` | take-ours + **mandatory post-merge verify** (never silent) |
| generated-artifact | `skills.generated.ts`, `aideck-consumer/schema.json`, `package-lock.json` | resolve → **regenerate** → assert-green |
| **everything else** | `*.js`, router/skill `*.md`, `*.test.js`, `package.json`, the `.atomic-skills/projects/**` tracking tree | **eject → human/LLM** |

**Discipline (Iron Law R-XAGENT-03 extended to the table):** the table auto-resolves ONLY
the proven-mechanical allowlist; every mechanical resolution emits a per-path **audit line**
(path · class · action). Genuine semantic source merges and the structural-**supersession
DECISION** (which whole side wins, e.g. an engine rewrite over old wiring) are **always**
human — the tool may offer "take-all-from-branch-X" but never infers the winner. The
`.atomic-skills/projects/**` tracking tree stays **eject** until the Decision-5 ownership
partition lands (silent N-way auto-merge of the tracking tree is blocked on it).

`merge=union` is also wired in `.gitattributes` for the append ledgers so they never even
conflict on a tree where the attribute is active; the driver's `union` policy is the
fallback for trees (or target repos) where it is not.
