...
| **G** | phase-done | Fixed order: tasks done → evaluation agent → stamp `phases[].evaluationGate` via `buildEvaluationGate` / `canRunPhaseDone` → **HARD-GATE:** `assert-automate-gate --gate phase-done` must exit 0 → **then** `phase-done` with `review-code --mode=both` (F2 wires transition default). **Not** `external-both` here — that mode is plan-end only (Step I). Durable decisions log visible for audit. Non-zero assert ⇒ do not run `phase-done`. |
...
- Fixed evaluation order: all phase tasks `done` → **evaluation agent** → stamp `evaluationGate` → `assert-automate-gate --gate phase-done` → phase-done `review-code --mode=both`.
...
- **Layer-2 assert (required before C/E/G/I):** `scripts/assert-automate-gate.js` — non-zero exit forbids advancing.
