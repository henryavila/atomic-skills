...
- Fixed evaluation order: all phase tasks `done` → **evaluation agent** → **write evaluationReport + stamp authentic `evaluationGate` (`reportPath` / `operatorSkip`)** → **distill lessons + operator ratify + stamp `lessonsState` (`recorded`+`lessonsPath` or explicit `none`)** → **`review-code --mode=both` + stamp `reviewGate`** → **decision-review** → **`audit-delivery` + stamp `deliveryAuditGate` (`reportPath` + `CLOSED|PARTIAL`; never skippable)** → **`assert-automate-gate --gate phase-done`** → terminal `phase-done` writes. Never assert before review + delivery audit; never forge `status: passed` without `reportPath`; never invent evaluation `skipped` without operator `operatorSkip`+reason; **never skip or operatorSkip `deliveryAuditGate`** (illegal); never omit lessons under automate (silence ≠ zero). Plan-end `intentVsDelivered` ≠ phase `deliveryAuditGate`.
