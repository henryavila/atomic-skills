...
6. **Plan-tree product fence:** under durable stamp, pass `--plan-diff-file` (paths from `git diff --name-only <baseRef>..HEAD`) or `--base-ref` so assert injects the path list into `planTreeProductFenceOk`. Product paths not covered by claim `paths[]` ⇒ assert done fails (`blocked: plan-tree product fence…`). State paths under `.atomic-skills/` do not trip the fence. No chat waiver.
