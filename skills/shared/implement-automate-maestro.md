...
| **G** | phase-done | Fixed order: tasks done → evaluation agent → stamp `phases[].evaluationGate` via `buildEvaluationGate` / `canRunPhaseDone` → **then** `phase-done` with `review-code --mode=both` (F2 wires transition default). **Not** `external-both` here — that mode is plan-end only (Step I). Durable decisions log visible for audit. |
...
- Fixed evaluation order: all phase tasks `done` → **evaluation agent** → stamp `evaluationGate` → phase-done `review-code --mode=both` (check `canRunPhaseDone` first).
