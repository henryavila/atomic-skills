...
6. **File lists:** arquivos/comandos/scripts listados existem? Execute {{GLOB_TOOL}} ou {{GREP_TOOL}} para confirmar — NÃO confie no nome. Se o plano diz "execute X", verifique que X existe. Para arquivos que uma task anterior do plano cria, verifique que a task de criação existe e vem antes.
...
### ITERAÇÃO 1:
1. Leia o plano inteiro com a ferramenta {{READ_TOOL}}. Aplique CADA item do checklist.
...
2. Leia o plano CORRIGIDO do início usando a ferramenta {{READ_TOOL}}
   (NÃO revisão mental — execute {{READ_TOOL}} no arquivo). Cite line numbers.
3. Verifique se:
...
- "Já li o plano inteiro mentalmente, não preciso usar {{READ_TOOL}} de novo"
...
| "Já verifiquei mentalmente" | Verificação mental não conta — execute {{READ_TOOL}} |
...
| "O arquivo provavelmente existe, o nome faz sentido" | Nomes que fazem sentido é como bugs se escondem. Execute {{GLOB_TOOL}} para confirmar |
...
**Chamadas {{READ_TOOL}} executadas:** [N]
