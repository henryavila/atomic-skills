...
export function DemoBanner({ visible }: Props) {
  ...
  return (
    <div
      style={{
        background:
          'linear-gradient(180deg, color-mix(in srgb, var(--status-blocked) 10%, var(--bg-canvas)), var(--bg-canvas))',
    ...
    >
      ...
      >
        Seeded fixtures, not your data.{' '}
        <code style={{ fontFamily: 'var(--font-mono)', color: 'var(--fg-default)' }}>Ctrl+C</code> to quit.
      </span>
    </div>
  )
}
