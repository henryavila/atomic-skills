...
export const SKILLS: Skill[] = [
  ...
  },
  {
    ...
    active: true,
    when: ["You finished a coherent code change", "You just implemented something and it is still uncommitted (wip scope)", "Significant change about to merge (both mode recommended)", "Critical path (auth, payments, data integrity) — both mode", "Cheap pre-merge sanity check (local mode)"],
    whenNot: ["Nothing to review (clean tree, no commits ahead of base)", "Trivial change already heavily reviewed", "Codex CLI not installed and you need codex mode (use --mode=local)"],
    examples: ["/atomic-skills:review-code", "/atomic-skills:review-code wip --mode=local", "/atomic-skills:review-code feat/new-feature --mode=local", "/atomic-skills:review-code main..HEAD --mode=both"],
    args: [
      { name: "git-ref", kind: "positional", required: false, description: "Git ref (branch, commit, a..b / a...b) or scope keyword: wip (uncommitted), branch (merge-base..HEAD), all (both). Empty → interactive scope picker." },
  ...
  },
  {
    ...
    subcommands: [
      ...
      { name: "verify", signature: "[--fix] [--slug <slug>]", description: "Reconcile .atomic-skills/ against the repo: schema, legacy-layout, branch match, scope coverage, orphans, aiDeck coherence (read-only unless --fix)", example: "/atomic-skills:project verify" },
      ...
      { name: "idea", signature: "[list | promote <n>]", description: "Capture a raw idea into the ideas.md inbox (fork: just save / analyze); `idea list` is a zero-token view; `idea promote <n>` routes idea #n through the emergence ladder (ratify-gated)", example: "/atomic-skills:project idea list" },
      ...
      { name: "reconcile", signature: "", description: "Close tasks/gates that look done in the repo — the ONLY completion-mutation path; verifier-aware (Run verifier when one exists, Mark done only when verifier-absent)", example: "/atomic-skills:project reconcile" },
      ...
      { name: "finalize", signature: "", description: "Publish the finished plan branch as a PR: push plan/<slug> + gh pr create --base <integrationRef>, record the PR url in plan state; operator-prompted, pre-merge, pre-archive", example: "/atomic-skills:project finalize" },
      ...
      { name: "migrate", signature: "[<slug>]", description: "Two modes: `migrate <slug>` converts a legacy (pre-0.1) initiative to schemaVersion 0.1 (field-mapping diff + placeholder flags); bare `migrate` runs the flat→projects/<id>/<slug>/ layout cut-over (deterministic copy-verify-delete behind a tar snapshot)", example: "/atomic-skills:project migrate" },
  ...
  },
  {
    ...
    versionAdded: "2.2.0",
    summary: "Turn an open idea into a committed, critic-approved design.md — the divergent front-half of DESIGN. Use before writing a multi-phase plan, when the WHAT/WHY and the chosen approach are not yet decided.",
    active: true,
    when: ["Starting a multi-phase plan whose approach is not yet decided", "There are ≥2 viable approaches and the decision is expensive to reverse", "You need a committed design.md before decomposing into tasks"],
    whenNot: ["An ad-hoc or single-task change (triage exempts it from DESIGN)", "The design is already committed and critic-approved", "You only need divergent perspectives, not a committed artifact (use debate)"],
    examples: ["/atomic-skills:brainstorm \"self-host the project lifecycle\"", "/atomic-skills:brainstorm"],
    args: [
      { name: "goal", kind: "positional", required: false, description: "The problem/goal to design. If omitted, the skill asks interactively." },
    ],
    dependencies: ["git"],
  ...
  },
  {
    ...
    args: [
      ...
      { name: "--gate", kind: "flag", required: false, description: "Gate-mode: bounded agenda + mandatory contrarian every round + a machine-readable Synthesis verdict block handed to a critic. Produces evidence for a stage gate; never decides." },
    ],
  ...
  {
    id: "implement",
    title: "Implement — Mode 1 Execution Driver",
    emoji: "⚙️",
    oneLiner: "Drive decomposed tasks to done, one at a time, verifier-gated",
    versionAdded: "2.3.0",
    summary: "Drive the SPEC-admitted tasks of a decomposed plan to DONE — the single-threaded execution driver at the tail of the lifecycle. Use to implement a plan task-by-task, closing each only through verify-on-done, with event-driven session handoffs that make a fresh session resume cheaply.",
    active: true,
    when: ["A plan has been decomposed and its tasks admitted by the SPEC gate", "You are implementing a phase task-by-task and want verifier-gated completion", "Resuming a prior implementation session from its handoff block"],
    whenNot: ["There is no plan/design yet (use brainstorm, then project new plan)", "A one-off bug fix with a known root cause (use fix)", "You only need to verify a single claim, not drive a plan (use verify-claim)"],
    examples: ["/atomic-skills:implement migration-self-host", "/atomic-skills:implement"],
    args: [
      { name: "plan-slug", kind: "positional", required: false, description: "The plan (or project-id/plan-slug) to implement. If omitted, uses the active plan/initiative." },
    ],
    dependencies: ["git"],
  ...
  },
  {
    ...
    versionAdded: "2.3.0",
    summary: "Turn a claim of completion (done / passes / fixed) into a verified fact from a fresh run, or a binary FAIL. Use before marking a task done, when an agent or a cheap/foreign executor reports success, or to gate any \"it works\" claim on the observed result instead of the narrative.",
    active: true,
    when: ["About to mark a task done and need its verifier run for real first", "An agent, subagent, or Codex reported success and you must adjudicate it", "Gating any \"the tests pass / the bug is fixed\" claim on captured evidence"],
    whenNot: ["The claim is a human-judgement / UI observation (use the manual-acceptance gate)", "There is no deterministic verifier (the task failed SPEC admission — surface it)", "You are diagnosing a bug, not verifying a fix (use fix)"],
    examples: ["/atomic-skills:verify-claim T-004", "/atomic-skills:verify-claim \"the parser handles empty input\""],
    args: [
      { name: "claim", kind: "positional", required: false, description: "The claim or task id under verification. If omitted, the skill asks for the claim and its verifier." },
    ],
    dependencies: ["git"],
