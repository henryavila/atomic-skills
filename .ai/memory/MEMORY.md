...
- [reference-aideck-card-failed-to-load.md](reference-aideck-card-failed-to-load.md) — Card "⊘ failed to load" = HTTP 400 de schema (ref sem `kind`), não bundle velho. Bundle vendorizado = main estável (sem normalize layer v2); rebuild não corrige. Fix é no dado.
