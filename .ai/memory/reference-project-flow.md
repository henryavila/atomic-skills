...
**Incidente 2026-08-28 (curta subject-aware-framing):** `project flow` abriu `http://127.0.0.1:<port>/flow.html`; a página falhou no primeiro load e depois mostrou a vista lista/card (o process-map). Causas: (1) L2 ainda era `<ol class="fl-messages">` / cards — `flow-draw.js` existia e não estava no produto; (2) `serve-flow --up` usava pipe+homedir(), no Windows o lock não isolava e o filho podia morrer com o `--up`. Correção: chrome SVG é o único L2; `serve-flow` recusa `map.html`, lock em HOME/USERPROFILE, daemon `stdio:ignore`+`windowsHide`; package.json sem scripts de process-map; renderer recusa escrever `map.html`. Re-rodar `project flow` regenera o HTML.
...
**Engine de diagrama (plano 2026-08-14):** `docs/plans/2026-08-14-flow-diagram-engine.md`. L2 = SVG. **Look A · Linha**. Motor = `scripts/lib/flow-layout.js` + `scripts/lib/flow-draw.js`. Prévia (`docs/plans/gen-flow-diagram-preview.mjs`) é só chrome: importa o motor, não redesenha. **PDTI = um fixture L1** (`fluxo-sugestao.json`); o motor não lê ids/rótulos PDTI, não classifica fim por regex de label, não corta prefixo «Líder». Colunas = medida da subárvore; join = fan-in>1 ou `type:join` uma vez; volta = aresta a ancestral, faixa esquerda por aresta; máquina = rank BFS + slot por rank (saída terminal com irmão que continua desce). Catálogo da prévia = dogfood + 12 grafos de topologia distinta (`docs/plans/preview-fixtures.mjs`) — os 20 XOR clones foram lixo. Chrome = app página inteira (grid `auto 1fr`, sizer+stage). Zoom = `width`/`height` do SVG (nunca `transform: scale`); persiste por processo (`as-flow-zoom:<slug>`). Ajuste alterna **À altura** / **À largura**. **PDF** = `{slug}-fluxo.pdf` (3 páginas, papel claro, anexo a requisito).

**PDF export (2026-08-31):** causa raiz de “blocos pretos” = `svgToJpeg` serializava o SVG sem o `<style>` da página (classes + `var(--*)`). Fix consistente: `FLOW_DIAGRAM_CSS` único (`flow-diagram-css.js`) + `flowPdfEmbeddedStyle()` (`flow-pdf.js`) com tokens light-paper hex (independente do tema da UI) e overrides de `color-mix`; `svgForPdf` clona o SVG, injeta `<style data-fl-pdf>`, só então rasteriza. Re-gerar `flow.html` após mudança de chrome (`node scripts/render-flow.js flow.json -o flow.html`).

**Sequência FECHADA (2026-08-14, operador):** wash XOR só no xor de fora (18%); `BLOCK_LEAD` 18px acima de ◇/ramo/join; wash atrás das lifelines; atores sticky + hover; chip 168×26 (topo pode parecer maior — aceito) com `STICK_PAD` 8px; loop-ref arco ~5px; sem número de fileira; sem popover.

**Fluxo + Estados (2026-08-14, em refino):** aba visível **Estados** (`data-tab=machines` permanece). Teclas `1`/`2`/`3` = Sequência/Fluxo/Estados (`kbd` na pílula). Arestas com cabeça de seta (polígono por cima do nó). BPM = tronco + colunas irmãs medidas; join uma vez abaixo do portão; volta tracejada warning à esquerda, uma faixa por aresta; texto da caixa/losango centralizado (`stackInBox` + `dominant-baseline=central`). Cartão de transição = **um** bloco: título (o que aconteceu; nota ` · ` só se não repetir o efeito) + pastilha Grava/E-mail/Avisa na mesma linha do efeito (sem wrap que parece lista). N laços no mesmo estado = lados distintos (1=topo; 2=esq/dir; 3+=topo+lados+base). `render-flow.js` ainda é lista (Tasks 7–8 do plano). Testes do motor: `tests/flow-layout.test.js` + `tests/flow-draw.test.js`. `machines.minItems: 1`; `--strict` = sha do atributo. Não commitar automate `prepare.json` / `sealed-brief.md`.

**Celular (2026-08-14):** chrome da prévia é app + dock. ≤860px ou toque sem hover: abas 34px em 3 colunas; zoom/PDF/tema no rodapé (32px); `100dvh` + safe-area; um dedo = scroll nativo; pinça = zoom do SVG; chave `as-flow-zoom:<slug>:m`; primeira visita e troca de aba = caber na largura. Tema = ícone no dock cicla automático → claro → escuro. Tailscale (tailnet): `https://crcmg005078.bream-goldeye.ts.net:10000/2026-08-14-flow-diagram-engine-style-preview.html` (`tailscale serve --https=10000` → `:8765`). Não resetar o serve 443/8443.

**Vários processos (2026-08-14, operador aprovou):** um plano = N arquivos; cada um = Sequência+Fluxo+Estados. Botão Processo (nome + contagem) abre lista com busca e rolagem. Persistência `as-flow-process:<plan>` + `#p=<slug>`. JSON extra em `docs/plans/preview-processes/`. Prévia = dogfood + 12 topologias distintas (14 packs). Olho no catálogo: linear, xor-3, and+join, nested xor, dois loops, event+subprocess, diamante, 3 laços, dois terminais pares, 2 máquinas, 5 atores.

**Próxima sessão:** olho nas 14 topologias da prévia (`node docs/plans/gen-flow-diagram-preview.mjs`). Ligar `render-flow.js` ao motor (`layout*`/`draw*Svg`) quando o operador pedir o L2 de produto. Sem voltar a desenhar no gerador da prévia.
