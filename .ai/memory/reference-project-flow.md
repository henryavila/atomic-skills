...
**Fluxo + Estados (2026-08-14, em refino):** aba visível **Estados** (`data-tab=machines` permanece). Teclas `1`/`2`/`3` = Sequência/Fluxo/Estados (`kbd` na pílula). Arestas com cabeça de seta (polígono por cima do nó). BPM = tronco + colunas irmãs; join uma vez (D2 abaixo de S3edit); volta S1e→D1 tracejada warning pela esquerda, sem haste compartilhada; texto da caixa/losango centralizado (`stackInBox` + `dominant-baseline=central`). Estados = cartão por transição (título = o que aconteceu; pastilha Grava/E-mail/Avisa = efeito; sem `kind · label → target` solto); dois laços em Pendente lado a lado (atualiza / segue sem editar) — laço = permanece no estado. Sequência FECHADA; Fluxo bem encaminhado; Estados ainda sob olho do operador. Sem TDD Tasks 1–8 até o operador pedir. `machines.minItems: 1`; `--strict` = sha do atributo. Não commitar automate `prepare.json` / `sealed-brief.md`.

**Celular (2026-08-14):** chrome da prévia é app + dock. ≤860px ou toque sem hover: abas 44px em 3 colunas; zoom/PDF no rodapé; `100dvh` + safe-area; um dedo = scroll nativo; pinça = zoom do SVG; chave `as-flow-zoom:<slug>:m`; primeira visita e troca de aba = caber na largura. Tema = ícone no título (círculo/sol/lua) cicla automático → claro → escuro — sem a faixa Sistema/Claro/Escuro. Tailscale (tailnet): `https://crcmg005078.bream-goldeye.ts.net:10000/2026-08-14-flow-diagram-engine-style-preview.html` (`tailscale serve --https=10000` → `:8765`). Não resetar o serve 443/8443.

**Próxima sessão:** continuar o refino visual de **Estados** (e Fluxo se o operador pedir) na prévia `docs/plans/2026-08-14-flow-diagram-engine-style-preview.html` (`node docs/plans/gen-flow-diagram-preview.mjs`). Depois extrair TDD para `flow-layout` / `flow-draw` / `render-flow`.
