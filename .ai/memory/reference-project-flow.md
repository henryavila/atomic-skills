...
**Fluxo + Estados (2026-08-14, em refino):** aba visível **Estados** (`data-tab=machines` permanece). Teclas `1`/`2`/`3` = Sequência/Fluxo/Estados (`kbd` na pílula). Arestas com cabeça de seta (polígono por cima do nó). BPM = tronco + colunas irmãs; join uma vez (D2 abaixo de S3edit); volta S1e→D1 tracejada warning pela esquerda, sem haste compartilhada; texto da caixa/losango centralizado (`stackInBox` + `dominant-baseline=central`). Cartão de transição = **um** bloco: título (o que aconteceu; nota ` · ` só se não repetir o efeito) + pastilha Grava/E-mail/Avisa na mesma linha do efeito (sem wrap que parece lista). Dois laços em Pendente lado a lado. Sequência FECHADA; Fluxo bem encaminhado; Estados ainda sob olho do operador. Sem TDD Tasks 1–8 até o operador pedir. `machines.minItems: 1`; `--strict` = sha do atributo. Não commitar automate `prepare.json` / `sealed-brief.md`.
...
**Vários processos (2026-08-14, operador aprovou):** um plano = N arquivos; cada um = Sequência+Fluxo+Estados. Botão Processo (nome + contagem) abre lista com busca e rolagem. Persistência `as-flow-process:<plan>` + `#p=<slug>`. JSON extra em `docs/plans/preview-processes/`. Prévia dogfood + 20 exemplos = 22 itens.

**Próxima sessão:** chrome móvel + seletor de processos aprovados. Continuar o olho em **Estados** (e Fluxo se pedir) na prévia `docs/plans/2026-08-14-flow-diagram-engine-style-preview.html` (`node docs/plans/gen-flow-diagram-preview.mjs`). Sem TDD Tasks 1–8 até o operador pedir. Depois extrair para `flow-layout` / `flow-draw` / `render-flow`.
