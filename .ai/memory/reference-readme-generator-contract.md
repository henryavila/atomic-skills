...
## Source of truth
- **Skills**: `meta/catalog.yaml` (renamed from `meta/skills.yaml` in the catalog-rename initiative). Core-only — no installer modules.
...
## Four marker-bounded regions in README.md
...
3. `<!-- SKILLS_TABLE_START/END -->` — overview table from `catalog.core`.
4. `<!-- SKILL_DETAILS_START/END -->` — per-skill detail blocks; Iron Law extracted from skill body files.

(Former `MODULES_*` markers / `module_meta` removed with the installer modules concept.)

## Enforcement
- `npm run check-docs` — exits non-zero if any region drifts.
- `npm run validate-skills` — hard-fails if catalog still has top-level `modules` or `module_meta`.
...
- **Field caps:** `one_liner` must be **10–80 chars** (validate-skills rejects otherwise). `replaceBetween` (render-readme.js) replaces ONLY content between the 4 marker pairs, so hand-written sections outside them are preserved and safe to edit directly — `check-docs` won't flag them.
...
## Adding a new skill
1. Add the entry under `core.<name>:` in catalog.yaml (or use `scripts/new-skill.js`).
2. Add the body at `skills/core/<name>.md` with a `## Iron Law` section.
